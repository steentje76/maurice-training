...
## v4.50.0 — Eerlijke app & gesloten coachinglus (19 augustus 2026)

**De AI-coach had geen enkele limiet per gebruiker.** Er was een modelallowlist en een tokenplafond per aanroep, maar niets hield één account tegen om de API-sleutel van de eigenaar in een middag leeg te trekken. Nieuw: `migratie_v450.sql` met de tabel `ai_usage` (RLS aan, géén policies — alleen de service-rol komt erbij) en twee SECURITY DEFINER-functies. Het optellen gebeurt **atomair** met `insert … on conflict do update … returning`, zodat parallelle verzoeken serialiseren in plaats van allemaal dezelfde oude stand te lezen; de ophoging gaat vóór de limiettoets, zodat een geweigerde poging óók meetelt. Grens: 60 aanroepen per dag en 900 per maand, instelbaar via omgevingsvariabelen — een expliciet productbesluit, onderbouwd in `docs/MASTERSPRINT-STATUS.md`. Ontbreekt de servicesleutel of is de migratie nog niet gedraaid, dan gaat de aanroep door mét een waarschuwing in de log: een quotumcontrole die de coach onbruikbaar maakt zodra de telling stuk is, is erger dan geen quotum.

**Een gepauzeerde begeleide training was onvindbaar.** De training werd wél bewaard, maar geen enkel scherm bood een weg terug: wie het scherm sloot, was zijn sets kwijt tot hij toevallig opnieuw dezelfde training startte. Nieuw: een hervat-kaart op Home met het aantal reeds gelogde sets, en een verwijderknop die dat aantal expliciet noemt voordat hij iets weggooit.

**"Je hebt nog geen trainingen" terwijl er iets misging.** `sbGet` gaf bij élke fout — netwerk, verlopen sessie, serverfout, time-out — een lege array terug, en de 54 aanroepplaatsen presenteerden dat als "geen gegevens". Dat is niet alleen verwarrend, het nodigt uit tot opnieuw invoeren van gegevens die er allang zijn. Opgelost zonder 54 losse aanpassingen: de teruggegeven array draagt nu een niet-enumereerbare statuseigenschap. Bestaande lezers zien exact dezelfde array — `length`, `map`, `JSON.stringify`, `for…in` veranderen niet — en schermen die het willen weten kunnen `tkDataFout(rows)` vragen. Home en Voortgang tonen een foutkaart met een opnieuw-knop in plaats van een lege staat.

**Een niet-bevestigd account werd opgeruimd mét zijn OAuth-tokens.** Accountverwijdering en de dagelijkse opruiming van niet-bevestigde accounts hielden allebei hun eigen kopie van de tabellenlijst bij, met in de code de aantekening "bij wijzigingen aan de een, ook de ander nalopen". Die is gegaan zoals zulke aantekeningen gaan: 30 tabellen tegenover 16. Onder de veertien die alleen in de ene lijst stonden zat `wearable_connections`, met het access- én refresh-token van de koppeling. Nu één gedeelde routine (`netlify/functions/_userData.js`) voor beide, inclusief de bijzondere gevallen: beide richtingen van gedeelde content, alleen persoonlijke apparatuurrijen (gym-inrichting van anderen blijft bestaan), alleen eigen persoonlijke oefeningen. Mislukte tabellen worden gemeld in plaats van verzwegen. Tegelijk had de dagelijkse opruiming géén toegangscontrole: het pad is raadbaar en elke POST startte een verwijderronde. Die staat nu open voor de geplande aanroep van Netlify en voor een handmatige aanroep met `CLEANUP_SECRET`; is die variabele niet gezet, dan verandert er niets en volgt er een waarschuwing in de log — een beveiliging die de dagelijkse taak stilzwijgend uitschakelt is geen verbetering.

**De coachinglus was nergens gesloten.** Na iedere training kreeg de sporter een opdracht mee voor de volgende keer ("Verhogen (+2,5 kg)") — en die verdween bij het herladen van het scherm. De volgende training werd los daarvan opnieuw afgeleid uit de vorige sessie, het schema en het herstel van dat moment. De sporter kon dus niet zien wat hem was opgedragen, niet of hij het had opgevolgd, en al helemaal niet waarom het voorschrift van vandaag er soms van afweek. Het bleek niet nodig om iets extra's op te slaan: sinds Sprint 18 reist per set een bewijssnapshot mee in `sets_detail`, mét de genomen beslissing en de regel-id — het werd alleen nooit teruggelezen. Nieuw: **`coaching_loop.v1`** leest het advies terug (uit dezelfde set waarop het afrondscherm het destijds baseerde) en legt het naast het gewicht dat vandaag toch al op het scherm staat: gevolgd, of afgeweken mét de reden die de app zelf al kende. Zonder vastgelegd advies blijft die regel leeg — een verzonnen "dit zei ik vorige keer" is erger dan zwijgen. Wat een volledige lus nog mist en welke productbesluiten daarvoor nodig zijn: `docs/ONTWERP-COACHINGLUS.md`.

**Openstaand voor de eigenaar:** `migratie_v450.sql` draaien in de Supabase SQL-editor, en `CLEANUP_SECRET` zetten in Netlify. Of `migratie_v447.sql` in productie gedraaid heeft was in deze sessie niet vast te stellen.

## v4.49.0 — Mastersprint: evidence, dataveiligheid en de coachinglus (19 augustus 2026)

Autonome roadmap-executie vanaf RC0. Geen nieuwe schermen; deze release maakt bestaande beloften waar en dicht wat de audits per fase aan het licht brachten. Volledige verantwoording in `docs/MASTERSPRINT-STATUS.md`.

**Trainingsduur wordt eindelijk vastgelegd — en levert één belasting over kracht én cardio.** De timer liep al sinds het starten van een training, maar de verstreken tijd werd bij het afronden nergens weggeschreven. Daardoor gaf `AthleteCore.unifiedLoad()` bewust `{beschikbaar:false, ontbreekt:['duur_per_sessie']}` zodra een dag zowel kracht als cardio bevatte, en stonden `duur` en de bijbehorende relaties in het variabelenregister al maanden op 'toekomstig'. Nieuw: `migratie_v449.sql` (nullable kolom `sessions.duration_s` met check-constraint), de rekenregel **`srpe.v1`** (Foster session-RPE: RPE × duur in minuten, in AU) en de aansluiting in het dagbeeld. `unifiedLoad` levert pas een AU-reeks als élke dag volledig meetbaar is; bij één onvolledige dag blijft hij leeg en meldt hij de dekkingsgraad. `load.v1` behoudt zijn betekenis. De app schrijft het veld alleen weg als de kolom aantoonbaar bestaat, zodat een nog niet gedraaide migratie geen enkele sessie kan doen mislukken.

**Het bewijsspoor werd gewist bij een correctie (P0, dataverlies).** `sets_detail` werd bij het bewerken van een sessie volledig opnieuw opgebouwd uit de invoervelden, en die kennen geen `evidence`. Eén correctie in de historie wiste daarmee de snapshots van álle sets van die sessie: de onveranderlijkheid die `decision.js` beschrijft gold alleen in het geheugen, niet op het schrijfpad. Nieuw: **`evidence_preserve.v1`** — de snapshot blijft staan zolang kg, reps en RPE ongewijzigd zijn en vervalt zodra de set wél wijzigt, want vals bewijs is erger dan geen bewijs.

**De AI kon een onmogelijk voorschrift in de database zetten (P0).** `parseProgrammaJSON` controleerde alleen ÓF er sets en reps stonden. Een afwijkend antwoord met "12 × 40 op RPE 15" ging rechtstreeks naar `program_block_exercises`. Nieuw: **`prescription_guard.v1`** als deterministische poort. Sets buiten 1..10 of reps buiten 1..50 weigeren het blok — bijknippen zou een voorschrift verzinnen dat niemand heeft bedoeld. Een RPE buiten de CR-10-schaal vervalt alleen. De grenzen staan uitsluitend in de engine.

**De offline-wachtrij was niet gebruiker-gescoped (P0, dataverlies én privacy).** Queue-items droegen geen eigenaar. Logde sporter A uit met openstaande items en logde B in op hetzelfde toestel, dan vuurde de flush af met B's JWT; omdat `sessions` geen `user_id` meestuurt (die wordt server-side gestempeld) belandden A's trainingen onder B. Elk item draagt nu de uid van de aanmaker; flush, badge en wachtrij-scherm tonen en versturen uitsluitend de eigen items. Items van een ander blijven staan tot die weer inlogt.

**Een mislukte wachtrij-schrijfactie telde als succes (P0, dataverlies).** `offlineQueueAdd` geeft `false` bij een IndexedDB-fout; alle negen aanroepplaatsen negeerden dat. `finishSession` telde de oefening dan als opgeslagen, wiste `sessionLog` en meldde "n oefeningen opgeslagen" terwijl de training nergens meer bestond. Datzelfde gold voor een geweigerde `sessions`-write.

**Elke mislukte token-refresh logde de sporter uit (P0).** `refreshAuthToken` gaf `false` bij een geweigerd refresh-token, maar net zo goed bij een 5xx of een afgebroken verbinding — en beide aanroepers reageerden met `clearAuthSession()`. Eén netwerkwissel betekende dus opnieuw inloggen, en omdat de wachtrij alleen flusht met een sessie bleef alles wat offline gelogd was staan. Koud opstarten zonder netwerk maakte de app zelfs volledig onbruikbaar. De functie geeft nu een status terug (`ok` / `verlopen` / `tijdelijk`); alleen `verlopen` rechtvaardigt uitloggen.

**Een mislukte service-worker-install wiste de complete offline-cache (P0).** `cache.addAll()` is atomair: faalde één van de twintig assets — bijvoorbeeld de cross-origin font-URL op een captive-portal-wifi — dan werd er niets gecachet, terwijl `activate` daarna de oude, wél gevulde cache verwijderde. De app was dan offline dood zonder melding. Assets worden nu per stuk geplaatst. Tegelijk: de navigatiehandler cachet geen captive-portal-pagina meer over de app-shell, en een mislukte JS-fetch krijgt een 504 in plaats van HTML.

**In de Android-app werkte geen enkele Netlify-functie (P0).** Alle 21 aanroepen gebruikten een relatief pad. Capacitor serveert de app vanaf `https://localhost`, dus resolveerden ze naar de lokale assetserver: AI-coach, wearable-sync, account verwijderen en het volledige teambeheer waren stuk in de app. Supabase werkte wel, want dat is een absolute URL — daarom viel het niet op. Opgelost met één `FN_BASE`-constante volgens hetzelfde patroon dat `sw.js` al gebruikt voor de video's, plus CORS aan de serverkant (`netlify/functions/_cors.js`) met een expliciete lijst oorsprongen en een preflight-antwoord. De JWT-verificatie van elke functie blijft ongewijzigd.

**Opgeslagen XSS via oefenings- en trainingsnamen (P0).** Vrije tekst werd in `onclick`-attributen geïnterpoleerd met escaping die daar niet werkt: `escHtml` wordt door de HTML-parser gedecodeerd vóór de JS-parser het ziet, en `JSON.stringify` ontsnapt geen apostrof. Namen zijn geen eigen invoer — een coach of gym-eigenaar maakt ze aan en ze zijn voor alle leden zichtbaar, met het sessie-JWT uit `localStorage` binnen bereik. Nieuw: `attrArg(v)` = `escHtml(JSON.stringify(v))`. Alle 24 plaatsen omgezet, inclusief het Beheer-scherm dat hier sinds v3.3.10 op de openstaande lijst stond.

**De privacybelofte werd niet nagekomen (P0).** De privacyverklaring zegt dat de AI-coach zonder toestemming niet wordt gebruikt, maar `ensureAiConsent()` werd maar op één plek aangeroepen. De vijf andere aanroepen — intake-extractie, twee programmageneratoren, de terugblik na een training en het programma-advies — stuurden gezondheids- en trainingsgegevens zonder die controle. Alle zes lopen nu via één poort.

**Recovery werd niet toegepast op eigen trainingen (P0).** `startCustomTraining` berekende de herstelaanpassing alleen in de tak zonder `resolvedItems`, en die tak is onbereikbaar geworden. Een eigen workout werd dus zonder aanpassing uitgevoerd terwijl een vaste training die wél kreeg. Daarmee verdween ook de tweede, afwijkende kopie van dezelfde regel.

**Een oefening toevoegen wiste de Builder-targets (P0, dataverlies).** `pushCustomTrainingExercises` deed DELETE + INSERT met alleen `exercise_id` en `sort_order`; aangeroepen op een Builder-training waren daarmee alle sets, reps, RPE, rusttijden en picks weg. Bovendien gebruikte hij niet-gequeuede schrijffuncties.

**De vierde safeguard: tegenstrijdige gegevens.** De app ving geen bewijs, lage betrouwbaarheid en ontbrekende gegevens al af. Gegevens die elkaar TEGENSPREKEN niet — een groene dagfactor naast "ik voel me slecht" leverde gewoon een groen advies op, even stellig als op een dag waarop alles klopte. Nieuw: **`conflict.v1`** met vijf expliciete tegenstrijdigheden, allemaal op drempels die elders al gelden. De regel verandert zone noch aanpassing; hij verlaagt alleen de zekerheid waarmee de uitkomst gepresenteerd mag worden, en de coach en de AI zijn meegewijzigd.

**"Waarom zegt de app dit?" wordt nu ook ná de training beantwoord.** De uitleg bestond al (`CoachingCore.explainProgression`) maar werd alleen tijdens de set getoond. Op het afrondscherm — waar de sporter zijn opdracht voor de volgende keer krijgt — stond alleen de opdracht. Hij komt nu uit dezelfde beslissing; zonder uitleg staat er geen regel.

**Betrouwbaarheid hoort in het bewijs.** Het snapshot legde vast WÁT er besloten was, niet hoe stevig dat besluit stond, terwijl `readinessDay` en `recoveryScore` die informatie al leverden. `buildDecisionEvidence` heeft nu een ingespoten `confidence`-veld; ontbreekt het, dan meldt de snapshot dat in `missing`.

**Eén volumeformule voor alle schermen.** Een sessierij bewaart een samenvatting (`weight` = zwaarste set), maar zeven schermen rekenden `sets × reps × weight` en overschatten daarmee structureel — terwijl het afrondscherm per set optelde. Dezelfde training leverde op vier plekken een ander getal op. Nieuw: **`session_volume.v1`**, dat per set uit `sets_detail` telt en alleen bij oudere rijen terugvalt op de samenvatting.

**Overige correcties.** De opslagindicator tijdens een training stond altijd op "✓ Alles opgeslagen" (schreef naar legacy-ids die niet meer bestaan). Een nieuwe sporter had na de onboarding geen enkele startknop; er staat nu een lege staat met "Maak je eerste training". Een mislukte check-in werd niet gemeld. Een gedeeltelijk mislukte sessie schreef bij een tweede poging de geslaagde oefeningen nogmaals weg. De intakevlag werd gezet ook als het profiel niet was opgeslagen. `sbDel` en `sbUpsert` liepen als enige buiten de 401-herstellaag om. De AI-proxy zette `model` en `max_tokens` ongecontroleerd door — nu allowlist en plafond. Tikdoelen: de gewichtknop tijdens een set (32×28) en de universele terugknop (36×36) naar 44×44.

**Artefact van 14 MB naar 5,1 MB.** `exercise-catalog.json` (296 kB) en `exercise-intelligence_6.json` (8,3 MB) werden meegebundeld terwijl geen enkele regel code ze ophaalt — 60% van wat de gebruiker downloadt was dood gewicht. Ze blijven in de repository als bron; alleen niet meer in de download. De Concept2-simulator, die `provenance: concept2_live_ble` op verzonnen metingen zet, staat niet langer op de browser-global en zit daarmee niet meer in het artefact.

**Tests.** 73 bestanden groen, 0 rood. Vijf nieuwe suites: `fSessieDuur` (35), `fBewijsAiGrens` (22), `fOfflineHardening` (24), `fPlatformGrens` (24), `fCoachingLus` (37). Geen test verwijderd of verzwakt; zeven contracttesten bewust bijgewerkt omdat het contract zelf veranderde (readiness-whitelist, evidence-snapshot, registersleutels, localStorage-classificatie, de architectuurlijst van schrijffuncties, de FN_BASE-aanroep en de volumeformule).
