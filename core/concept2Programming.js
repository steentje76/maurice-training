...
(function (global) {
  ...
  var TERMINAL = [STATE.CONFIRMED, STATE.FAILED, STATE.TIMEOUT, STATE.CANCELLED];
  ...
  function createProgrammingController(deps) {
    ...
    function programFixedDistance(distanceMeters, ctx) {
      ...
      var op = {
        ...
        frame: built.frame, startedAt: now(), timer: null, resolve: null
      };
      pending = op;
      ...
      Promise.resolve(writeFn(built.frame)).then(function () {
        ...
        op.timer = setT(function () {
          if (pending === op) settle(STATE.TIMEOUT, 'no_response_within_timeout');
        }, timeoutMs);
      }).catch(function (e) {
    ...
    function handleControlResponse(bytes, ctx) {
      ...
      if (parsed.previousFrameStatus === S.OK) { settle(STATE.CONFIRMED, null); return { handled: true, state: STATE.CONFIRMED }; }
