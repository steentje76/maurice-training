...
ok(contract.includes('ARCHITECTURE READY — IMPLEMENTATION OPEN'),
  'A1: het contract is expliciet gestatust als ARCHITECTURE READY, niet als geimplementeerd');
...
// Bevestig dat er GEEN nieuwe migratie is die deze tabellen daadwerkelijk aanmaakt
// (bewust niet live uitgevoerd conform de opdracht).
const migratieBestanden = fs.readdirSync(ROOT).filter(function (f) { return /^migratie_v5(2[89]|[3-9]\d)\.sql$/.test(f); });
...
ok(!bevatEnduranceTabellen,
  'C1: geen enkele recente migratie maakt de endurance-tabellen daadwerkelijk aan -- het contract blijft bewust op ontwerpniveau (ARCHITECTURE READY, geen premature implementatie)');
