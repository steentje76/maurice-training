...
const mCur  = finishBron.match(/const curPr=\(sessionPrBase\[ex\.id\]!==undefined\?sessionPrBase\[ex\.id\]:\(prFor\(ex\.id\)\|\|0\)\);/);
ok(!!mBase, 'basislijn-expressie letterlijk gevonden');
ok(!!mCur, 'vergelijkings-expressie letterlijk gevonden');
...
function simuleer(dbPr, setGewichten){
  ...
  const curPr = eval('(' + mCur[0].replace(/^const curPr=/, '').replace(/;$/, '') + ')'); // ECHTE regel uit finishSession
  let isPr = false;
  if (best > curPr) { db.sq = best; isPr = true; }
...
ok(/if\(w>curPr\)\{ await upsertExerciseGoalField\(it\.id,'pr',w\); \}/.test(html),
   'E1: Guided Execution schrijft de PR nog op de eigen, ongewijzigde manier');
ok(/row\.weight>\(prFor\(exId\)\|\|0\)\) await upsertExerciseGoalField\(exId,'pr',row\.weight\)/.test(html),
   'E2: losse oefening schrijft de PR nog op de eigen, ongewijzigde manier');
