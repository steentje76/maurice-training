/* fTrainingExecutionScenarioBDE.test.js — V1 Proven Maturity Sprint 05B.
 * Training Execution Completion Certification -- Scenario B (offline vlak
 * vóór afronden) en Scenario E (meerdere reconnect-cycli binnen één
 * trainingssessie). SIMULATED-FAILURE-TESTED (niet PHYSICAL-DEVICE-TESTED).
 * Hergebruikt de Sprint-04/05-methodiek (LETTERLIJK uit index.html
 * geëxtraheerde productiecode), lokaal opnieuw geëxtraheerd -- geen
 * cherry-pick van PR #268.
 */
'use strict';
const fs = require('fs');
...
(async () => {
  ...
  // ═══ SCENARIO B: offline vlak vóór afronden ═══
  {
    const mockServerRows = { training_instances: [] };
    let net = 'ONLINE';
    const sandbox = buildSandbox(mockServerRows, () => net);
    // buildSandbox's applyServerWrite ondersteunt geen PATCH-op-bestaand-id
    // door zichzelf -- we breiden de mock-fetch uit met een PATCH-tak die
    // een bestaande rij (gevonden via url) op status 'completed' zet,
    // exact zoals een echte PostgREST-PATCH zou doen.
    ...
    sandbox.fetch = async function (url, init) {
      if (init.method === 'PATCH') {
        ...
        if (row) { row.status = 'completed'; row.completed_at = row.completed_at || new Date().toISOString(); }
        return { ok: true, status: 200, json: async () => ({}) };
      }
      return origFetch(url, init);
    };
    const mod = runInSandbox(src, sandbox);
    // 1. training starten + 2 sets ONLINE geregistreerd (laatste set is al binnen).
    const tiId = crypto.randomUUID();
    await mod.sbPostQ('training_instances', { id: tiId, user_id: 'user-A', status: 'active' });
    await mod.sbPostQ('sessions', { id: null, user_id: 'user-A', exercise_id: 'squat', reps: 5, weight: 100 });
    await mod.sbPostQ('sessions', { id: null, user_id: 'user-A', exercise_id: 'squat', reps: 5, weight: 105 });
    // 2. netwerk uit vlak vóór afronden.
    net = 'OFFLINE';
    // 3. gebruiker kiest afronden -- completeTrainingInstance() -> sbPatchQ -> queuet.
    const finishResult = await mod.sbPatchQ('training_instances', 'id=eq.' + tiId, { status: 'completed', completed_at: new Date().toISOString() });
    xok(finishResult === true, 'Scenario B stap 3: de afrond-PATCH wordt gequeued (true, geen zichtbare fout) i.p.v. de finish-flow te laten crashen');
    const wachtrijVoorReconnect = await mod.offlineQueueAll();
    xok(wachtrijVoorReconnect.length === 1 && wachtrijVoorReconnect[0].method === 'PATCH', 'Scenario B: de queued afronding is zichtbaar in de wachtrij (herstelbaar, niet stil verloren)');
    xok(mockServerRows.training_instances[0].status === 'active', 'Scenario B: vóór reconnect staat de server-rij nog op "active" (geen vals succes voorgewend)');
    // 9-10. netwerk herstellen + flush.
    net = 'ONLINE';
    await mod.flushOfflineQueue();
    xok(mockServerRows.training_instances.length === 1, 'Scenario B (backend): exact 1 training_instances-record na reconnect (geen duplicate/incomplete history)');
    xok(mockServerRows.training_instances[0].status === 'completed', 'Scenario B: de training staat na reconnect correct op "completed"');
    xok((mockServerRows.sessions || []).length === 2, 'Scenario B: beide vóór het offline-moment geregistreerde sets blijven behouden (geen verlies)');
    const wachtrijNa = await mod.offlineQueueAll();
    xok(wachtrijNa.length === 0, 'Scenario B: wachtrij leeg na succesvolle flush');
  }

  // ═══ SCENARIO E: meerdere reconnect-cycli binnen één trainingssessie ═══
  {
    const mockServerRows = { training_instances: [] };
    let net = 'ONLINE';
    const sandbox = buildSandbox(mockServerRows, () => net);
    ...
    net = 'OFFLINE';
    await mod.sbPostQ('sessions', { id: null, user_id: 'user-A', exercise_id: 'squat', reps: 5, weight: 105 });
    await mod.sbPostQ('sessions', { id: null, user_id: 'user-A', exercise_id: 'bench', reps: 8, weight: 60 });
    // online -> flush
    net = 'ONLINE';
    await mod.flushOfflineQueue();
    // offline -> nieuwe write
    net = 'OFFLINE';
    await mod.sbPatchQ('training_instances', 'id=eq.' + tiId, { status: 'completed' });
    // online -> flush
    net = 'ONLINE';
    await mod.flushOfflineQueue();

    xok((mockServerRows.sessions || []).length === 3, 'Scenario E: alle 3 sets (1 online, 2 tijdens offline) staan uiteindelijk op de server');
    xok(mockServerRows.training_instances.length === 1 && mockServerRows.training_instances[0].status === 'completed', 'Scenario E: exact 1 training_instances-record, correct afgerond na de laatste flush-cyclus');
    const wachtrijEind = await mod.offlineQueueAll();
    xok(wachtrijEind.length === 0, 'Scenario E: wachtrij convergeert naar exact 0 na de laatste reconnect-cyclus');
  }

  console.log('fTrainingExecutionScenarioBDE: ' + p + ' geslaagd, ' + f + ' mislukt');
