/* core/runningExecution.js — B9-02B Running Core Closure.
 *
 * Canonieke, pure execution state machine + timer-engine voor Running.
 * Geen DOM/database/network-toegang (Calculation/Decision Core purity,
 * consistent met core/calculation.js en core/decision.js).
 *
 * KRITIEK (sectie 5 van de opdracht): setInterval() is NOOIT de bron van
 * waarheid voor verstreken tijd. Alle tijd wordt deterministisch afgeleid
 * uit timestamps + een lijst van segmenten (actief/gepauzeerd). Een
 * UI-laag mag setInterval() gebruiken om de weergave elke seconde te
 * verversen, maar moet daarvoor telkens elapsedActiveMs() opnieuw
 * aanroepen -- nooit een lokale teller ophogen.
 *
 * State machine (sectie 4):
...
(function (root, factory) {
  ...
  var VERSIONS = { execution: 'running_execution.v1' };
