...
- **F5 — Connected Athlete: CURRENT (expliciet vrijgegeven door de Product Owner, 29 augustus 2026; Product Owner langere tijd niet beschikbaar — autonome uitvoering binnen de canonieke F5-scope, geen bewijs verzonnen).** MS-F5-01: **CLOSED**. MS-F5-02: **dimensionaal — Software TESTED/CLOSED, Integration OPEN, Real Device OPEN** (geldige eindstatus, geen bewijs verzonnen). MS-F5-03 (Android Health Connect Production Path): **CLOSED, met een belangrijke, eerlijke architectuurbevinding** — repo-brede audit + actueel webonderzoek bevestigen dat Trainingskompas uitsluitend de **Google Health API** (cloud, server-side OAuth, opvolger van de sunsettende Fitbit Web API) gebruikt, **geen native Android Health Connect-SDK** (fundamenteel ander product: on-device datastore zonder cloud-API). De bestaande architectuur zelf is end-to-end gevalideerd (minimale read-only scopes, 7-dagen-syncgrens, idempotente atomaire upsert, no-wearable-garantie — allemaal bevestigd). Een eventuele toekomstige, aanvullende Health Connect-SDK-integratie is vastgelegd als een expliciet product-besluitpunt in `docs/F5_PRODUCT_OWNER_DECISIONS.md`, niet zelf besloten. **F5 open P1: 0.** Resterende F5-mastersprints (MS-F5-04 t/m 06) nog niet uitgevoerd.
...
- **Tests:** discovery-based release gate (lokaal én CI, schone checkout) — 110 testbestanden in `core/` ontdekt (+ `logic_tests.js` + 2 statische checks = 113 stappen totaal), 112 automatisch uitgevoerd, 1 zichtbaar geskipt (fAndroidRelease.test.js zonder gereproduceerde buildmap), 1 zichtbaar geskipt (`fAndroidRelease.test.js`, ontbrekende Android-buildmap in deze schone checkout — draait wél in echte CI via `npm run cap:copy`, en lokaal na een expliciete reproductie: dan 102/102, 0 geskipt), 0 gefaald
