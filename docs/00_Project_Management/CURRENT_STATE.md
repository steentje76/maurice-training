...
- **F5 — Connected Athlete: CURRENT (expliciet vrijgegeven door de Product Owner, 29 augustus 2026; Product Owner langere tijd niet beschikbaar — autonome uitvoering binnen de canonieke F5-scope, geen bewijs verzonnen).** MS-F5-01 (Provider Integration Contract): **CLOSED**. MS-F5-02 (Concept2 PM5 Real-device Validation): **dimensionaal — Software TESTED/CLOSED, Integration OPEN, Real Device OPEN** (geldige eindstatus conform de opdracht: geen Android-buildruntime en geen fysieke PM5 beschikbaar in deze sandbox-omgeving, geen bewijs verzonnen). Belangrijke, functioneel bewezen bevinding: de BLE-verbindingslaag is volledig ontkoppeld van de trainingssessie-levenscyclus — een disconnect kan een actieve training nooit beëindigen/wissen/resetten (de twee lagen delen geen enkel code-pad, sterker dan een losse guard). Bestaande native-transport-implementatie (`native/src/nativeConcept2BleTransport.js`) bevestigd echt, geen stub. GAP-P2-020 (multiple HR-bronnen tegelijk, niet-kritiek) geregistreerd. **F5 open P1: 0** (beide afgeronde sprints zijn eerlijk niet-blokkerend afgesloten). Resterende F5-mastersprints (MS-F5-03 t/m 06) nog niet uitgevoerd.
...
- **Tests:** discovery-based release gate (lokaal én CI, schone checkout) — 109 testbestanden in `core/` ontdekt (+ `logic_tests.js` + 2 statische checks = 112 stappen totaal), 111 automatisch uitgevoerd, 1 zichtbaar geskipt (fAndroidRelease.test.js zonder gereproduceerde buildmap), 1 zichtbaar geskipt (`fAndroidRelease.test.js`, ontbrekende Android-buildmap in deze schone checkout — draait wél in echte CI via `npm run cap:copy`, en lokaal na een expliciete reproductie: dan 102/102, 0 geskipt), 0 gefaald
