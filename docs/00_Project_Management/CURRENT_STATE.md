...
- **F9 — Social & Community: CURRENT (expliciet vrijgegeven door de Product Owner, 30 augustus 2026).** MS-F9-01: **CLOSED** — vier kritieke security-bugs live gevonden en gerepareerd (anonieme toegang, block-RLS-omzeiling, self-role-elevation, anon-EXECUTE-lek), geen 6e bottom-nav-tab (genuine navigatieconflict). MS-F9-02 (Clubs, Groups & Challenges): **CLOSED** — `SocialGroupCore`/`SocialChallengeCore` gebouwd, apart datamodel van de commerciële organizations/teams-architectuur (geen cross-model privilege-vertaling). Groups: kleinst mogelijk rolmodel (owner/member), de zelf-join-elevation-les uit MS-F9-01 direct toegepast (rolwijziging uitsluitend via owner-gebonden `SECURITY DEFINER`-check, live geverifieerd). Challenges: Social is uitsluitend een **consument** van canonieke trainingsdata — V1 ondersteunt één metric (`completed_sessions_count`, pure telling), acht health-gerelateerde metrics (gewicht/calorieën/hartslag/HRV/slaap/afstand/volume/e1RM) expliciet uitgesloten en getest. Timezone-safe `DATE`-kolommen voor challenge-periodes (voorkomt het GAP-P3-004-patroon). Live adversarial getest: self-elevation en block-bypass (ook tegenover de challenge-creator) beide correct geweigerd. **F9 open P1: 0.** Resterende F9-mastersprint (MS-F9-03) nog niet uitgevoerd.
...
- **Tests:** discovery-based release gate (lokaal én CI, schone checkout) — 134 testbestanden in `core/` ontdekt (+ `logic_tests.js` + 2 statische checks = 137 stappen totaal), 136 automatisch uitgevoerd, 1 zichtbaar geskipt (fAndroidRelease.test.js zonder gereproduceerde buildmap), 0 gefaald
