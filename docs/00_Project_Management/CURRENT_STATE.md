...
- **F11 — Gym/Club/Team Platform: CURRENT (expliciet vrijgegeven door de Product Owner, 30 augustus 2026).** MS-F11-01 (Organization & Location Core): **CLOSED** — baseline-audit bevestigde twee parallelle organisatiemodellen: legacy `gyms`/`users.gym_id`/PIN-gebaseerd (actief, enkel-tenant, blijft ongewijzigd bestaan) en het nieuwere `organizations`/`memberships`/`teams`/`training_groups` (multi-tenant-klaar, had vóór deze sprint uitsluitend SELECT-policies). Het ontbrekende schrijf-fundament toegevoegd, plus een nieuwe `locations`-tabel. **Drie kritieke, live gevonden en gerepareerde security-bugs**: (1) `memberships` stond self-role-elevation toe naar 'owner' voor elke organisatie (PR #150, apart gemergd vóór de sprint zelf); (2) een owner van meerdere organisaties kon een lid verplaatsen tussen organisaties via een simpele UPDATE (tenant escape); (3) hetzelfde patroon voor `locations`. Beide laatste gerepareerd met BEFORE UPDATE-triggers die `organization_id`/`user_id`/`team_id` onvoorwaardelijk onveranderlijk maken na aanmaak. Delete-completeness gerepareerd (`organizations.owner_user_id` had geen CASCADE). Volledige adversarial matrix bewezen (multi-org role isolation, cross-tenant delete-bescherming, removed-membership verliest direct rechten). **F11 open P1: 0.** Volledig rapport: `docs/F11_MS_F11_01_REPORT.md`. Resterende F11-mastersprints (MS-F11-02 t/m 05) nog niet uitgevoerd.
...
- **Tests:** discovery-based release gate (lokaal én CI, schone checkout) — 147 testbestanden in `core/` ontdekt (+ `logic_tests.js` + 2 statische checks = 150 stappen totaal), 149 automatisch uitgevoerd, 1 zichtbaar geskipt (fAndroidRelease.test.js zonder gereproduceerde buildmap), 0 gefaald
