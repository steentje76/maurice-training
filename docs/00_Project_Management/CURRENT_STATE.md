...
- **F6 — Endurance & Multisport Excellence: CURRENT (expliciet vrijgegeven door de Product Owner, 29 augustus 2026; Product Owner langere tijd niet beschikbaar — autonome uitvoering binnen de canonieke F6-scope).** MS-F6-01: **CLOSED**. MS-F6-02: **CLOSED, met een kritieke, cross-cutting AI-boundary-fix** (4 sport-coachingteksten gecorrigeerd). MS-F6-03 (Rowing & Erg Intelligence): **CLOSED — een hergebruik-sprint, geen nieuwe berekeningslogica nodig.** `CardioCore.criticalPower()` (uit MS-F6-02) en `ProgressionCore.trendBy()` (uit F4) bleken beide volledig sportagnostisch en direct herbruikbaar voor Concept2-ergdata (handmatig plausibiliteitsgeverifieerd met realistische 2k/500m-ergprestaties) — geen tweede, gedupliceerde CP- of trendfunctie gebouwd. Bevestigd: de MS-F6-02-coachingtekst-fix voor roeien staat nog correct. **F6 open P1: 0.** Resterende F6-mastersprints (MS-F6-04 t/m 06) nog niet uitgevoerd.
...
- **Tests:** discovery-based release gate (lokaal én CI, schone checkout) — 116 testbestanden in `core/` ontdekt (+ `logic_tests.js` + 2 statische checks = 119 stappen totaal), 118 automatisch uitgevoerd, 1 zichtbaar geskipt (fAndroidRelease.test.js zonder gereproduceerde buildmap), 0 gefaald
