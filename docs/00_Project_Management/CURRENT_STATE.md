...
- **F4 — Coach Intelligence: CURRENT (expliciet vrijgegeven door de Product Owner, 29 augustus 2026).** MS-F4-01: TESTED. MS-F4-02, MS-F4-03, MS-F4-04: CLOSED. **F4 open P1: 0** (AI-PROGRAM-AUTOGEN-001 CLOSED via MS-F4-04). MS-F4-05 (canonieke naam: **Schedule & Missed-workout Adaptation**, track Decision & Rules Engine): **CLOSED** — runtime-trace bevestigde `ScheduleAdherenceCore` (Program Adaptation V1) bestond al, puur deterministisch, actief gebruikt op meerdere schermen inclusief binnen `buildCtx()` (event-nabijheid voedt de AI-context). Conflict-detectie nooit stille overschrijving, bevestigd via sabotagebewijs. Resterende F4-mastersprint (MS-F4-06) nog niet uitgevoerd.
...
- **Tests:** discovery-based release gate (lokaal én CI, schone checkout) — 106 testbestanden in `core/` ontdekt (+ `logic_tests.js` + 2 statische checks = 109 stappen totaal), 108 automatisch uitgevoerd, 1 zichtbaar geskipt (fAndroidRelease.test.js zonder gereproduceerde buildmap), 1 zichtbaar geskipt (`fAndroidRelease.test.js`, ontbrekende Android-buildmap in deze schone checkout — draait wél in echte CI via `npm run cap:copy`, en lokaal na een expliciete reproductie: dan 102/102, 0 geskipt), 0 gefaald
