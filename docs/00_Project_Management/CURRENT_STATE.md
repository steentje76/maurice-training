...
- **F10 — Coach/PT Platform: CURRENT (expliciet vrijgegeven door de Product Owner, 30 augustus 2026; Product Owner tijdens uitvoering beschikbaar voor productbeslissingen).** MS-F10-01 (Coach Consent & Permissions): **CLOSED** — Baseline-audit bevestigde dat `coach_athlete_relationships` al bestond, correct en veilig ontworpen, maar volledig ongekoppeld (0 runtime-referenties, geen dedicated test, geen granulaire toestemmingslaag). **Granulaire coach-toegangsscopes toegevoegd conform Product Owner-beslissing**: `CoachAccessCore` + `coach_access_scopes`-tabel met drie scopes — TRAINING_CORE (default aan), RECOVERY_HEALTH (default uit), WOMENS_PERFORMANCE (default uit, **altijd apart**, nooit impliciet via de andere twee scopes of social/team/gym/premium-relaties). Server-side default-scope-trigger (nooit client-side te manipuleren). **Zes kritieke scenario's live adversarial geverifieerd** op de productiedatabase: coach zonder relatie (0 toegang), self-elevation-poging (RLS-schending), actieve relatie + defaults (uitsluitend TRAINING_CORE zichtbaar), Women's Performance blijft geweigerd zelfs met beide andere scopes aan, revoke stopt toegang onmiddellijk, geen cross-coach-lek. **Genuine bevinding gecorrigeerd**: het bestaande `delete-account.js`-commentaar claimde ten onrechte dat nergens `ON DELETE CASCADE` naar `auth.users` bestaat — `coach_athlete_relationships` heeft dit wél; commentaar gecorrigeerd, tabel alsnog expliciet in de verwijderlijst opgenomen. **F10 open P1: 0.** Resterende F10-mastersprints (MS-F10-02, MS-F10-03, MS-F10-04) nog niet uitgevoerd.
...
- **Tests:** discovery-based release gate (lokaal én CI, schone checkout) — 138 testbestanden in `core/` ontdekt (+ `logic_tests.js` + 2 statische checks = 141 stappen totaal), 140 automatisch uitgevoerd, 1 zichtbaar geskipt (fAndroidRelease.test.js zonder gereproduceerde buildmap), 0 gefaald
