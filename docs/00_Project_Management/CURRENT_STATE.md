...
- **F4 — Coach Intelligence: CURRENT (expliciet vrijgegeven door de Product Owner, 29 augustus 2026).** **Alle 6 F4-mastersprints afgerond:** MS-F4-01 TESTED; MS-F4-02, MS-F4-03, MS-F4-04, MS-F4-05, MS-F4-06 CLOSED. **F4 open P1: 0.** MS-F4-06 (canonieke naam: **Longitudinal Program Adaptation & Benchmark Tracking**, P2, "niet expliciet in v1.1 maar niet strijdig"): **CLOSED** — bijgewerkt, actueel webonderzoek (augustus 2026) bevestigde een belangrijk, nieuw feit: Hevy Trainer is expliciet een algoritme, geen AI/LLM, en gebruikt geen slaap/HRV/rusthartslag als programmeer-input. Eerlijke, genuanceerde herpositionering vastgelegd (`docs/MS-F4-06_LONGITUDINAL_BENCHMARK_TRACKING.md`) — TK-differentiators (herstelbewuste readiness, audit trail, AI-outputgovernance, lift-by-lift-stagnatiedetectie) mét expliciete erkenning van Hevy se voorsprong op programma-autogeneratie-schaal. Geen overbouwde infrastructuur voor dit P2-documentatie-item. F4 Final Integration Audit en F4 Master Report nog uit te voeren.
...
- **Tests:** discovery-based release gate (lokaal én CI, schone checkout) — 107 testbestanden in `core/` ontdekt (+ `logic_tests.js` + 2 statische checks = 110 stappen totaal), 109 automatisch uitgevoerd, 1 zichtbaar geskipt (fAndroidRelease.test.js zonder gereproduceerde buildmap), 1 zichtbaar geskipt (`fAndroidRelease.test.js`, ontbrekende Android-buildmap in deze schone checkout — draait wél in echte CI via `npm run cap:copy`, en lokaal na een expliciete reproductie: dan 102/102, 0 geskipt), 0 gefaald
