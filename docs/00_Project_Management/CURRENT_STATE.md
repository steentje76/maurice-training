...
- **F5 — Connected Athlete: CURRENT (expliciet vrijgegeven door de Product Owner, 29 augustus 2026; Product Owner langere tijd niet beschikbaar — autonome uitvoering binnen de canonieke F5-scope, geen bewijs verzonnen).** MS-F5-01: **CLOSED**. MS-F5-02: **dimensionaal**. MS-F5-03: **CLOSED**. MS-F5-04: **CLOSED**. MS-F5-05 (Wearable Provider Feasibility Matrix): **CLOSED** — alle 5 providers (Garmin/Polar/WHOOP/Suunto/COROS) onderzocht via actuele, officiële developerbronnen (geen scraping). Kernbevindingen: **Polar en COROS hebben de hoogste toegangs-feasibility** (self-serve resp. recent geopend, objectief onboardingproces); **Garmin is momenteel feitelijk ontoegankelijk** (aanmeldingen gerapporteerd stilgelegd, aanvraagformulier verwijderd) — vastgelegd als een tweede product-besluitpunt in `docs/F5_PRODUCT_OWNER_DECISIONS.md`; WHOOP vereist een eigen apparaat/lidmaatschap voor de ontwikkelaar; Suunto is discretionair en business-only. Evidence-based prioritering opgesteld zonder een commerciële beslissing te nemen. **F5 open P1: 0.** Resterende F5-mastersprint (MS-F5-06) nog niet uitgevoerd.
...
- **Tests:** discovery-based release gate (lokaal én CI, schone checkout) — 112 testbestanden in `core/` ontdekt (+ `logic_tests.js` + 2 statische checks = 115 stappen totaal), 114 automatisch uitgevoerd, 1 zichtbaar geskipt (fAndroidRelease.test.js zonder gereproduceerde buildmap), 1 zichtbaar geskipt (`fAndroidRelease.test.js`, ontbrekende Android-buildmap in deze schone checkout — draait wél in echte CI via `npm run cap:copy`, en lokaal na een expliciete reproductie: dan 102/102, 0 geskipt), 0 gefaald
