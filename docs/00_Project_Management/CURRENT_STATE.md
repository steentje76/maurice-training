...
- **F10 — Coach/PT Platform: CURRENT (expliciet vrijgegeven door de Product Owner, 30 augustus 2026; Product Owner tijdens uitvoering beschikbaar voor productbeslissingen).** MS-F10-01: **CLOSED** — `CoachAccessCore` + `coach_access_scopes` (TRAINING_CORE/RECOVERY_HEALTH/WOMENS_PERFORMANCE, WOMENS_PERFORMANCE altijd apart). MS-F10-02: **CLOSED** — `CoachRosterCore`, roster bevat uitsluitend actieve relaties. MS-F10-03 (Coach Programming & Assignment): **CLOSED** — een eerste implementatiepoging (coach schrijft direct in `programs`) bleek niet te werken door de bestaande `trg_set_user_id`-trigger; live adversarial bevestigd, direct gecorrigeerd, vastgelegd als **GAP-P2-023 (inmiddels volledig gesloten)**. Finale architectuur: `coach_program_templates` (coach-owned) → `coach_program_assignments` (koppeling) → athlete-geïnitieerde `materialize_coach_assignment()`-RPC die de volledige inhoud (dagen/oefeningen) materialiseert in de bestaande, canonieke keten (`programs`/`program_blocks`/`custom_trainings`/`training_exercises`) — de bestaande `trg_set_user_id`-trigger blijft volledig intact en doet zijn normale werk voor alle vier tabellen. Server-side exercise-ID-validatie tegen de canonieke Exercise Library. **Acht kritieke scenario's live adversarial bewezen**: volledige flow (alle vier eigenaarschapskolommen correct op de athlete), coach kan niet zelf materialiseren, onbekend exercise_id geweigerd, atomiciteit (midden-in-het-proces-fout laat 0 sporen achter), idempotentie op de volledige content, revoke laat een reeds gematerialiseerd programma intact terwijl nieuwe coach-toegang direct stopt, delete-completeness bevestigd. **Calendar/adherence bewezen zonder enige codewijziging**: `AdherenceIntelligenceCore.aggregate()` verwerkt de gematerialiseerde `program_blocks`-structuur correct. **F10 open P1: 0.**
