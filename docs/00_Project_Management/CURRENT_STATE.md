...
- **F9 — Social & Community: CURRENT (expliciet vrijgegeven door de Product Owner, 30 augustus 2026).** MS-F9-01 (Social Identity & Privacy Foundation): **CLOSED** — Information Architecture-audit bevestigde een genuine navigatieconflict (bestaande bottom-navigatie heeft al 5 vaste hoofdtabs) tegen de gesuggereerde "aparte Social-hoofdtab"-standaard; beslissing: Social krijgt een eigen scherm zonder de bestaande navigatie te wijzigen. `SocialPrivacyCore` (privacy-/relatiemodel: private/connections/discoverable, asymmetrisch follow, block-wint-altijd) gebouwd en getest. Datamodel (`social_profiles`/`social_connections`/`social_blocks`/`social_reports`) live toegepast met RLS. **Vier kritieke, live ontdekte en gerepareerde security-bugs** via verplichte adversarial testing op de productiedatabase: (1) anonieme toegang tot discoverable profielen, (2) block-omzeiling via RLS-subquery-isolatie (SECURITY DEFINER-fix), (3) self-role-elevation in social_connections (de follower kon zichzelf accepteren), (4) onbedoeld anon-EXECUTE-recht op de SECURITY DEFINER-functie. Alle vier live herbevestigd als gerepareerd, vóórdat er ooit UI op gebouwd werd. **F9 open P1: 0.** Resterende F9-mastersprints (MS-F9-02, MS-F9-03) nog niet uitgevoerd.
...
- **Tests:** discovery-based release gate (lokaal én CI, schone checkout) — 130 testbestanden in `core/` ontdekt (+ `logic_tests.js` + 2 statische checks = 133 stappen totaal), 132 automatisch uitgevoerd, 1 zichtbaar geskipt (fAndroidRelease.test.js zonder gereproduceerde buildmap), 0 gefaald
