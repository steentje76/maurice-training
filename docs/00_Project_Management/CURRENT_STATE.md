...
- **F9 — Social & Community: CURRENT (F9 SOCIAL & COMMUNITY CLOSED — READY FOR F10 SELECTION; F10 nog niet vrijgegeven).** Alle 3 F9-mastersprints afgerond. F9 Final Integration Audit: **kritieke P1-bevinding gevonden en direct gerepareerd** — geen van de tien nieuwe F9-social-tabellen (`social_profiles`, `social_connections`, `social_blocks`, `social_reports`, `social_groups`, `social_group_memberships`, `social_challenges`, `social_challenge_participants`, `social_shared_activities`, `social_notifications`) stond in `delete-account.js`; zes daarvan hebben geen standaard `user_id`-kolom (afwijkende/dubbele eigenaarskolommen), waardoor een simpele lijst-toevoeging niet had volstaan — nu correct opgelost met kolom-specifieke verwijderblokken (analoog aan het bestaande `content_shares`-patroon), `core/fDeleteAccountSecurity.test.js` uitgebreid naar 22/22 met sabotagebewijs. Alle security-suites herbevestigd groen (150 tests: RLS multi-tenant, coach-proxy, wearable-auth, observability, plus vier nieuwe F9-social-RLS-suites). Self-elevation-, block-bypass-, shadow-calculation-, causale-taal- en Women's-Performance-isolatie-audits: 0 bevindingen. **F9 open P1: 0.** Volledig rapport: `docs/F9_MASTER_REPORT.md`. **F10 vereist een nieuwe, expliciete vrijgave van de Product Owner.**
