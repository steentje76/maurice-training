...
- **F10 — Coach/PT Platform: CURRENT (expliciet vrijgegeven door de Product Owner, 30 augustus 2026; Product Owner tijdens uitvoering beschikbaar voor productbeslissingen).** MS-F10-01: **CLOSED** — `CoachAccessCore` + `coach_access_scopes` (TRAINING_CORE/RECOVERY_HEALTH/WOMENS_PERFORMANCE, WOMENS_PERFORMANCE altijd apart), zes kritieke scenario's live adversarial geverifieerd. MS-F10-02: **CLOSED** — `CoachRosterCore`, roster bevat uitsluitend actieve relaties, overview toont uitsluitend toegestane secties. MS-F10-03 (Coach Programming & Assignment): **IMPLEMENTED, niet CLOSED** — een eerste implementatiepoging (coach schrijft direct in `programs` met `user_id=athlete`) bleek niet te werken: de bestaande `trg_set_user_id`-trigger dwingt onvoorwaardelijk `user_id=auth.uid()` af, live adversarial bevestigd, direct gecorrigeerd (geen lek), vastgelegd als **GAP-P2-023**. Product Owner-architectuurbeslissing: drie gescheiden verantwoordelijkheden — `coach_program_templates` (coach-owned), `coach_program_assignments` (koppeling), athlete-owned materialisatie via de nieuwe `materialize_coach_assignment()`-RPC (uitsluitend aanroepbaar door de athlete zelf; de bestaande trigger blijft volledig intact en doet zijn normale werk). Vier kritieke scenario's live adversarial geverifieerd: volledige flow correct, coach kan niet zelf materialiseren, idempotentie werkt, derde partij kan de assignment niet eens lezen. **Eerlijk vastgelegde, resterende stap**: de daadwerkelijke content-materialisatie (template-inhoud naar `program_blocks`/geplande trainingsdagen) is nog niet gebouwd — een gematerialiseerd programma is vooralsnog leeg. Calendar/adherence werken architecturaal al (hergebruik van bestaande `ScheduleAdherenceCore`/`AdherenceIntelligenceCore`, geen tweede engine), maar pas zichtbaar zodra er daadwerkelijk geplande content is. **F10 open P1: 0.** MS-F10-04 nog niet gestart (dependency MS-F10-03 nog niet volledig CLOSED).
...
- **Tests:** discovery-based release gate (lokaal én CI, schone checkout) — 142 testbestanden in `core/` ontdekt (+ `logic_tests.js` + 2 statische checks = 145 stappen totaal), 144 automatisch uitgevoerd, 1 zichtbaar geskipt (fAndroidRelease.test.js zonder gereproduceerde buildmap), 0 gefaald
