...
- **F10 — Coach/PT Platform: CURRENT (expliciet vrijgegeven door de Product Owner, 30 augustus 2026; Product Owner tijdens uitvoering beschikbaar voor productbeslissingen).** **Alle 4 F10-mastersprints afgerond.** MS-F10-01: **CLOSED** — `CoachAccessCore` + granulaire scopes (TRAINING_CORE/RECOVERY_HEALTH/WOMENS_PERFORMANCE, WOMENS_PERFORMANCE altijd apart). MS-F10-02: **CLOSED** — `CoachRosterCore`, roster bevat uitsluitend actieve relaties, geen enumeratie. MS-F10-03: **CLOSED** — na een genuine, live gevonden en direct gecorrigeerde architecturale blokkade (**GAP-P2-023, inmiddels volledig gesloten**: de bestaande `trg_set_user_id`-trigger verhinderde een naïeve coach-direct-insert-aanpak), finale architectuur: coach-owned templates → assignment → athlete-geïnitieerde materialisatie-RPC die de volledige trainingsinhoud in de bestaande, canonieke keten plaatst (`programs`/`program_blocks`/`custom_trainings`/`training_exercises`), met server-side exercise-ID-validatie, bewezen atomiciteit en idempotentie. Calendar/adherence bewezen zonder codewijziging via de bestaande `AdherenceIntelligenceCore`. MS-F10-04 (Coach Intelligence): **CLOSED** — `CoachIntelligenceCore`, een whitelist-contract (analoog aan `WomensPerformanceContextCore`) dat uitsluitend reeds-canonieke outputs (adherence/plateau/readiness) selecteert voor een AI-samenvatting aan de coach, gefilterd via `CoachAccessCore`; Women's Performance blijft ook hier altijd apart, nooit afgeleid van de andere scopes. Geen nieuwe berekening, geen shadow-decision, expliciet onderscheid van de AI-coach-proxy (`coach.js`). **F10 open P1: 0.** F10 Final Integration Audit en F10 Master Report volgen.
