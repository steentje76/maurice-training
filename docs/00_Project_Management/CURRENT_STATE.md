...
- **APP_VER:** v4.69.10 (zie "Huidige versie" hierboven — exacte kop vereist door `core/fAndroidRelease.test.js` H2, Wet 84-versiebumpcontrole; niet wijzigen zonder die test aan te passen)
...
- **F5 — Connected Athlete: CURRENT (expliciet vrijgegeven door de Product Owner, 29 augustus 2026; Product Owner langere tijd niet beschikbaar — autonome uitvoering binnen de canonieke F5-scope, geen bewijs verzonnen).** MS-F5-01 (Provider Integration Contract): **CLOSED** — connector-inventaris opgesteld (Google Health enige actieve wearable-provider; Concept2 pure-core aanwezig maar native BLE-transport niet in deze repo — real-device-validatie categorisch onmogelijk in deze omgeving; weer/overige providers nog niet geïmplementeerd). Canonieke keten-wet (device→adapter→raw+provenance→normalisatie→kwaliteit→canoniek model→Calculation→Context→Decision→AI→atleet) bevestigd al aanwezig in bestaande code-commentaren, niet nieuw verzonnen. Live DB-geverifieerd: RLS deny-all op tokentabellen (`wearable_connections`/`wearable_oauth_state`, RLS ingeschakeld met 0 policies = correct, veilig deny-all-gedrag), `UNIQUE(user_id,provider)` + merge-duplicates-upsert (connectie-idempotency), `UNIQUE(user_id,date)` + atomaire RPC (data-idempotency, F3-erfenis). Twee nieuwe, niet-kritieke P2-gaps geregistreerd (GAP-P2-018: geen granulair per-waarde quality-veld; GAP-P2-019: geen retry-met-backoff). Resterende F5-mastersprints (MS-F5-02 t/m 06) nog niet uitgevoerd.
...
- **Tests:** discovery-based release gate (lokaal én CI, schone checkout) — 108 testbestanden in `core/` ontdekt (+ `logic_tests.js` + 2 statische checks = 111 stappen totaal), 110 automatisch uitgevoerd, 1 zichtbaar geskipt (fAndroidRelease.test.js zonder gereproduceerde buildmap), 1 zichtbaar geskipt (`fAndroidRelease.test.js`, ontbrekende Android-buildmap in deze schone checkout — draait wél in echte CI via `npm run cap:copy`, en lokaal na een expliciete reproductie: dan 102/102, 0 geskipt), 0 gefaald
