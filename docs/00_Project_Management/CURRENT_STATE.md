...
- **F3 — Calculation/Context/Evidence Excellence: CURRENT.** MS-F3-01 t/m MS-F3-03: CLOSED. MS-F3-04 (Endurance & Erg Calculation Registry): **PARTIAL** (bewust, geen vals CLOSED) — pace/power/erg metrics volledig gedekt en gevalideerd; Critical Speed/Power, TRIMP, aerobic decoupling en HR-zones bestaan niet en zijn expliciet NOT_IMPLEMENTED geregistreerd (bevestigd bestaand architectuurcommentaar in intervalEngine.js: "Geen FTP/critical power/critical speed" — geen ontdekte omissie, een eerdere bewuste keuze). Vereist een productbeslissing over methode-keuze vóór implementatie, geen technische lacune. Resterende F3-mastersprints (MS-F3-05 t/m 11) nog niet uitgevoerd. Zie `docs/CALCULATION_REGISTRY.md`.
...
- **Tests:** discovery-based release gate (lokaal én CI, schone checkout) — 93 testbestanden in `core/` ontdekt (+ `logic_tests.js` + 2 statische checks = 96 stappen totaal), 95 automatisch uitgevoerd, 1 zichtbaar geskipt (`fAndroidRelease.test.js`, ontbrekende Android-buildmap in deze schone checkout), 0 gefaald
