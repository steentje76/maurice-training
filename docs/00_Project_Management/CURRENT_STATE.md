...
- **F3 — Calculation/Context/Evidence Excellence: CURRENT.** MS-F3-01 t/m MS-F3-03: CLOSED. MS-F3-04 (Endurance & Erg): status TESTED (inhoudelijk gedeeltelijk — pace/power/erg metrics volledig gevalideerd, CS/CP/TRIMP/HR-zones bewust NOT_IMPLEMENTED, geen vals CLOSED). MS-F3-05 (Energy & Estimate Registry): **CLOSED** — kernbevinding: TK berekent zelf geen energieverbruik (geen MET/BMR/TDEE-formule), dus kan nooit een eigen precisie-claim doen boven wat de bron rapporteert; alle calorie-/BMR-waarden waren al correct als extern (USER_REPORTED/WEARABLE_ESTIMATE) gelabeld. Resterende F3-mastersprints (MS-F3-06 t/m 11) nog niet uitgevoerd. Zie `docs/CALCULATION_REGISTRY.md`.
...
- **Tests:** discovery-based release gate (lokaal én CI, schone checkout) — 94 testbestanden in `core/` ontdekt (+ `logic_tests.js` + 2 statische checks = 97 stappen totaal), 96 automatisch uitgevoerd, 1 zichtbaar geskipt (`fAndroidRelease.test.js`, ontbrekende Android-buildmap in deze schone checkout), 0 gefaald
