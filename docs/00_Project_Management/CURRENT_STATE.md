...
- **F10 — Coach/PT Platform: CURRENT (expliciet vrijgegeven door de Product Owner, 30 augustus 2026; Product Owner tijdens uitvoering beschikbaar voor productbeslissingen).** MS-F10-01: **CLOSED** — `CoachAccessCore` + `coach_access_scopes` (TRAINING_CORE/RECOVERY_HEALTH/WOMENS_PERFORMANCE, WOMENS_PERFORMANCE altijd apart), zes kritieke scenario's live adversarial geverifieerd, CASCADE-documentatie gecorrigeerd. MS-F10-02 (Coach Roster & Athlete Overview): **CLOSED** — `CoachRosterCore` bouwt voort op de reeds bewezen RLS + `CoachAccessCore`: het roster bevat uitsluitend athletes met een actieve relatie (geen enumeratie mogelijk, pending/revoked verschijnen nooit), en de athlete-overview toont uitsluitend secties waarvoor daadwerkelijk toestemming bestaat (geen "vergrendeld"-placeholder die de inhoud zou verraden). Onafhankelijk, opnieuw geverifieerd na de MS-F10-01-merge (alle testsuites + live `anon`-EXECUTE-controle herbevestigd). **F10 open P1: 0.** Resterende F10-mastersprints (MS-F10-03, MS-F10-04) nog niet uitgevoerd.
...
- **Tests:** discovery-based release gate (lokaal én CI, schone checkout) — 139 testbestanden in `core/` ontdekt (+ `logic_tests.js` + 2 statische checks = 142 stappen totaal), 141 automatisch uitgevoerd, 1 zichtbaar geskipt (fAndroidRelease.test.js zonder gereproduceerde buildmap), 0 gefaald
