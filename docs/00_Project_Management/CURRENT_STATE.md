...
- **F5 — Connected Athlete: CURRENT (expliciet vrijgegeven door de Product Owner, 29 augustus 2026; Product Owner langere tijd niet beschikbaar — autonome uitvoering binnen de canonieke F5-scope, geen bewijs verzonnen).** MS-F5-01: **CLOSED**. MS-F5-02: **dimensionaal — Software TESTED/CLOSED, Integration OPEN, Real Device OPEN**. MS-F5-03: **CLOSED** (belangrijke bevinding: TK gebruikt uitsluitend de Google Health API, geen native Android Health Connect-SDK — vastgelegd als product-besluitpunt). MS-F5-04 (Apple HealthKit Architecture): **CLOSED** — volledig, greenfield architectuurontwerp opgesteld op basis van actuele, officiële Apple-documentatie (geen bestaande iOS-scaffold, geen implementatie gebouwd of voorgewend, conform de acceptance gate "iOS path designed before implementation"). Kernbevindingen: HealthKit is fundamenteel platform-beperkt (geen REST/OAuth, data verlaat het apparaat nooit zonder expliciete upload — een andere architectuurcategorie dan alle overige F5-providers); de officiële HK-type-identifiers zijn geverifieerd; de HRV-methodologie-nuance (Apple's SDNN-gebaseerde meting) is expliciet vastgelegd, niet stilzwijgend gelijkgesteld aan de Google Health API's HRV. **F5 open P1: 0.** Resterende F5-mastersprints (MS-F5-05, MS-F5-06) nog niet uitgevoerd.
...
- **Tests:** discovery-based release gate (lokaal én CI, schone checkout) — 111 testbestanden in `core/` ontdekt (+ `logic_tests.js` + 2 statische checks = 114 stappen totaal), 113 automatisch uitgevoerd, 1 zichtbaar geskipt (fAndroidRelease.test.js zonder gereproduceerde buildmap), 1 zichtbaar geskipt (`fAndroidRelease.test.js`, ontbrekende Android-buildmap in deze schone checkout — draait wél in echte CI via `npm run cap:copy`, en lokaal na een expliciete reproductie: dan 102/102, 0 geskipt), 0 gefaald
