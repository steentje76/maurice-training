...
- **F3 — Calculation/Context/Evidence Excellence: CURRENT.** MS-F3-01 t/m MS-F3-03: CLOSED. MS-F3-04 (Endurance & Erg): status TESTED (inhoudelijk gedeeltelijk, bewust NOT_IMPLEMENTED CS/CP/TRIMP/HR-zones). MS-F3-05 (Energy): CLOSED. MS-F3-06 (Context Taxonomy & Contract): **CLOSED** — kernbevinding: `ContextEngineCore` (core/contextEngine.js) is dode code, de daadwerkelijke contextbron is `buildCtx()` (index.html), die correct aan canonieke calculaties delegeert en de AI-grens expliciet in de prompttekst afdwingt ("wijzig het advies niet, vul niets in"). Resterende F3-mastersprints (MS-F3-07 t/m 11) nog niet uitgevoerd. Zie `docs/CONTEXT_CONTRACT.md`.
...
- **Tests:** discovery-based release gate (lokaal én CI, schone checkout) — 95 testbestanden in `core/` ontdekt (+ `logic_tests.js` + 2 statische checks = 98 stappen totaal), 97 automatisch uitgevoerd, 1 zichtbaar geskipt (`fAndroidRelease.test.js`, ontbrekende Android-buildmap in deze schone checkout), 0 gefaald
