...
- **F3 — Calculation/Context/Evidence Excellence: CURRENT.** MS-F3-01 t/m MS-F3-03: CLOSED. MS-F3-04: TESTED. MS-F3-05 t/m MS-F3-10: CLOSED. MS-F3-11 (Formal Calculation & Evidence Specification): **CLOSED** — `docs/CALCULATION_EVIDENCE_SPEC.md` (nieuw) consolideert alle F3-registries tot één canoniek engineering-contract; geen nieuwe technische claims, uitsluitend bestaande, al-geverifieerde bevindingen geformaliseerd. **Alle 11 F3-mastersprints nu afgerond** (MS-F3-04 blijft eerlijk op TESTED, geen kunstmatige CLOSED). Tijdens de finale integratie-audit een nieuwe, live-bevestigde bevinding gedaan: GAP-P1-008 (bewezen, niet theoretisch race-condition-risico op `hrv_log` — zie hieronder). Zie `docs/CALCULATION_EVIDENCE_SPEC.md` en het F3 Master Report.
