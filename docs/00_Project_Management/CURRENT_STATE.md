...
- **F9 — Social & Community: CURRENT (expliciet vrijgegeven door de Product Owner, 30 augustus 2026).** **Alle 3 F9-mastersprints afgerond:** MS-F9-01 (vier kritieke security-bugs live gevonden en gerepareerd; geen 6e bottom-nav-tab, genuine navigatieconflict), MS-F9-02 (`SocialGroupCore`/`SocialChallengeCore`, apart datamodel van de commerciële organizations/teams-architectuur; V1-challenge-metric uitsluitend `completed_sessions_count`, acht health-gerelateerde metrics expliciet uitgesloten). MS-F9-03 (Sharing, Moderation & Notifications): **CLOSED** — `SocialSharingCore`: een gedeelde activiteit is uitsluitend een referentie naar `training_instances` + een expliciete presentatiewhitelist, geen tweede waarheid. **Kritieke, live geverifieerde garantie:** block wint altijd, ook over publieke zichtbaarheid. Notifications: minimaal model, forgeer-veilig (geen insert-policy voor gewone gebruikers, uitsluitend `service_role`). Hergebruikt `social_reports` (MS-F9-01), geen tweede reportmodel. Eerlijk vastgelegde scope-beperkingen: push/e-mail-infrastructuur en reactions/comments bewust niet gebouwd (niet vereist door de letterlijke acceptance gate "Safe interaction layer"). **Genuine bevinding tijdens deze sprint:** een valse-positief in het bestaande `core/fFase2.test.js` gevonden en verantwoord gerepareerd (statement-niveau i.p.v. bestandsbreed regel-niveau matching), met sabotagebewijs dat de onderliggende garantie (geen ongeldige `training_instances`-statuswaarden) volledig intact blijft. **F9 open P1: 0.** F9 Final Integration Audit en F9 Master Report volgen.
...
- **Tests:** discovery-based release gate (lokaal én CI, schone checkout) — 136 testbestanden in `core/` ontdekt (+ `logic_tests.js` + 2 statische checks = 139 stappen totaal), 138 automatisch uitgevoerd, 1 zichtbaar geskipt (fAndroidRelease.test.js zonder gereproduceerde buildmap), 0 gefaald
