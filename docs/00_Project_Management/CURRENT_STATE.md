...
- **F2 — Athlete Core Excellence: CURRENT.** MS-F2-01 (Canonical Training Start & Preview): PARTIAL — 2 defecten gefixed, volledige convergentie blijft open (GAP-P1-006). MS-F2-02 (Execution Reliability & Persistence) en MS-F2-03 (Workout Builder & My Trainings): **CLOSED** — beide audits bevestigen dat de bestaande code al zorgvuldig gehard is; geen nieuwe defecten gevonden, bestaande garanties vastgelegd in `core/fExecutionReliability.test.js`/`core/fWorkoutBuilder.test.js`. Zie `docs/MS-F2-02_EXECUTION_RELIABILITY.md`/`docs/MS-F2-03_WORKOUT_BUILDER.md`.
...
- **Tests:** discovery-based release gate (lokaal én CI, schone checkout) — 83 testbestanden in `core/` ontdekt (+ `logic_tests.js` + 2 statische checks = 86 stappen totaal), 85 automatisch uitgevoerd, 1 zichtbaar geskipt (`fAndroidRelease.test.js`, ontbrekende Android-buildmap in deze schone checkout — draait wél in echte CI via `npm run cap:copy`), 0 gefaald
