...
- **F11 — Gym/Club/Team Platform: CURRENT (expliciet vrijgegeven door de Product Owner, 30 augustus 2026).** MS-F11-01: **CLOSED** — schrijffundament (organizations/teams/training_groups/memberships/locations), drie kritieke security-bugs live gevonden en gerepareerd (self-role-elevation, twee tenant-escape-bugs via immutabele tenant-identifiers). MS-F11-02 (Gym Programming & Equipment): **CLOSED** — hergebruikt volledig de bestaande F10 Coach Programming-architectuur (`coach_program_templates`/`coach_program_assignments`/`materialize_coach_assignment()`, uitgebreid met een organization-tak, geen tweede workoutmodel) en de bestaande `equipment_catalog`/`exercise_equipment` (uitgebreid met `organization_id`, geen tweede equipment-model). **Twee genuine, pre-existing bevindingen** in het legacy equipment-systeem gedocumenteerd en gerepareerd (een schijnbare RLS-bypass die door een bestaande trigger werd gecompenseerd; een nooit-werkende policy op `exercise_equipment`, bevestigd ongebruikt). **Twee eigen implementatiefouten live ontdekt en direct gecorrigeerd** vóór enige externe zichtbaarheid (trigger-voorrangsvolgorde tussen de twee organisatiemodellen; een assignment-policy die per ongeluk de verkeerde gebruiker controleerde). Volledige adversarial matrix bewezen: gym-template-toewijzing werkt correct binnen de organisatie, wordt geweigerd cross-tenant en voor niet-staff-leden, en een gematerialiseerd programma blijft bestaan na verwijdering uit de organisatie. **F11 open P1: 0.** Volledige rapporten: `docs/F11_MS_F11_01_REPORT.md`, `docs/F11_MS_F11_02_REPORT.md`. Resterende F11-mastersprints (MS-F11-03 t/m 05) nog niet uitgevoerd.
