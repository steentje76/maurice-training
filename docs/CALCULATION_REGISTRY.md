...
| Version | `hrv_baseline.v1` (HRV Canonicalization Sprint — voorheen intern ongenummerd) |
| Formula | `lnRmssd(v) = ln(v)` (v>0); baseline = gemiddelde + SD van `lnRmssd`-waarden over een venster; SWC = 0,5×SD; classificatie 'g'/'o'/'r'/'ref' op basis van het rollend 7-daags gemiddelde t.o.v. baseline±SWC; `direction` ('above'/'within'/'below'/'ref', additief, wijzigt nooit st/factor) geeft apart aan of de recente waarde boven, binnen, of onder het baseline-gemiddelde ligt |
| Implementation | `core/calculation.js` — `lnRmssd`, `hrvBaseline`, `hrvRollingRecent`, `hrvStPersonal`, `hrvDagFactorPersonal` (HRV Canonicalization Sprint: verhuisd uit index.html; index.html bevat nu uitsluitend dunne CalcCore-wrappers, geen eigen berekening) |
...
| Evidence level | **B** voor de Ln-RMSSD-transformatie + rollend-gemiddelde + SWC-methodologie (gevestigd, veelgeciteerd in de HRV-guided-training-literatuur); **C** voor de exacte TK-vensters (14/28 dagen, min. 4 metingen) — een eigen productkalibratie binnen die methodologie, niet rechtstreeks uit één paper overgenomen; **D** voor de `HRV_SEVERE_DROP_PCT=0.15`-drempel — expliciet een **PRODUCT HEURISTIEK**, bron is consumentencontent (athletedata.health), geen peer-reviewed publicatie. Deze drie claims worden bewust NIET tot één gezamenlijk cijfer samengevoegd. |
...
| Scientific sources | Plews DJ, Laursen PB, Stanley J, Kilding AE, Buchheit M. "Training adaptation and heart rate variability in elite endurance athletes: opening the door to effective monitoring." *Sports Medicine*. 2013;43(9):773-781 (onafhankelijk herbevestigd, HRV Canonicalization Sprint: Ln-RMSSD normaliseert de verdeling bij getrainde duursporters, rollend-gemiddelde+SWC=0,5×SD is de dominante, meest-geciteerde aanpak in de literatuur — exact de in de code geïmplementeerde methode). Plews et al. (2017, PubMed 24334285): minimaal 3 geldige metingen/week nodig voor een betrouwbaar weekgemiddelde — TK's `HRV_BASELINE_MIN_N=4` is conservatiever dan dit gepubliceerde minimum. |
| Limitations | HRV kent aanzienlijke dag-tot-dag-variabiliteit door meetcondities (houding, tijdstip, cafeïne — niet gestandaardiseerd in deze app, geen meetcontext-metadata); bij zeer hoge individuele HRV kan een "parasympathetic saturation"-fenomeen de lineaire aanname verstoren (Plews et al., 2013) — nu WEL herkenbaar via het `direction:'above'`-veld, maar bewust NIET automatisch als negatief signaal geïnterpreteerd (onvoldoende evidence voor een automatische regel); een verminderde dag-tot-dag-variabiliteit (Ln-RMSSD-CV) kan een vroeg non-functioneel-overreaching-signaal zijn (los van het gemiddelde-niveau) — dit is een aparte grootheid die TK nog niet berekent (kandidaat voor een toekomstige, apart te onderbouwen CALC-toevoeging); provenance is beperkt tot `manual`/`wearable`/`unknown` (geen apparaatmerk), dus device-specifieke betrouwbaarheidsweging (bv. Oura vs. Garmin) is nog niet mogelijk |
| Applicability | uitsluitend sporters met voldoende, regelmatige HRV-metingen; geen enkele claim tijdens de referentiefase |
| Forbidden interpretations | **hard vereist** (opdracht-guardrail): HRV alleen mag nooit overtraining diagnosticeren, een medische toestand vaststellen, een verplichte rustdag afdwingen, of blessure voorspellen. Code-implementatie bevestigt dit: `hrvDagFactorPersonal` levert uitsluitend een dagfactor-COMPONENT (0.85-1.05), nooit een zelfstandig advies. Het additieve `direction`-veld mag NOOIT automatisch worden geïnterpreteerd als "goed"/"slecht" bij 'above' — puur beschrijvend. |
...
| Test status | `core/fHrvBaselineCanonicalization.test.js` (dedicated, 64/64) + `core/fRecoveryRegistry.test.js`/`core/fCalculationRegistryCoverage.test.js` (structureel, bijgewerkt naar de canonieke locatie) |
| Status | **VERIFIED** (HRV Canonicalization Sprint: geëxtraheerd naar `core/`, dedicated getest, expliciet versienummer, eerder self-identified testgat gesloten) |
...
| Waarde | Locatie | Classificatie |
|---|---|---|
| `HRV_BASELINE_MIN_DAYS=14`, `HRV_BASELINE_FULL_DAYS=28`, `HRV_BASELINE_MIN_N=4` | core/calculation.js | **Evidence-backed** (consistent met Plews et al.'s aanbeveling van minimaal 3-5 metingen/week) |
| `HRV_SWC_MULTIPLIER=0.5` | core/calculation.js | **Evidence-backed** (Plews et al., letterlijk "0.5 SD as the smallest worthwhile change") |
| `HRV_SEVERE_DROP_PCT=0.15` | core/calculation.js | **Product heuristic** — code citeert zelf "athletedata.health" (een coaching-webbron, geen peer-reviewed studie) — terecht NIET als evidence-backed geclassificeerd, ondanks de nabijheid van de wél sterk onderbouwde SWC-drempel in dezelfde functie |
