...
| Endurance-profiel drempels (`enduranceProfielTekst`) | `athlete_endurance_profile` via één `sbGet` (`sport=in.(running,cycling)&limit=2`) — `threshold_pace_seconds_per_km` (running) en `ftp_watts_user_entered` (cycling) | **USER_REPORTED** (door de sporter ingevoerd via `upsert_endurance_profile_target`; geen berekende waarde, expliciet gescheiden van Critical Speed/Power — Gap 1a, Fase B1) | **slow-changing** | AI-promptcontext, uitsluitend voor de actieve sport (running/hardlopen, cycling/wielrennen, triathlon beide); ontbrekend = expliciet "niet ingesteld ... niet schatten" | Geen (geen confidence verzonnen; formattering via canonieke `CardioCore.formatTime`) |
| Endurance-intelligence (`enduranceIntelTekst`) | `tkEnduranceCoachContext()` — dunne adapter (Hyrox-precedent): `activities` begrensd (`sport=in.(actieve sporten)&recorded_at=gte.<84 d>&limit=200`) + `activities&is_max_effort=eq.true&limit=50`; uitsluitend canonical cores: `CardioCore.criticalSpeed/criticalPower` (critical_speed.v1 / critical_power.v1 via eligibility CALC-RUN-CSELIG-001 / CALC-CYC-CPELIG-001), `RunningIntelligenceCore.weeklyVolume` (CALC-RUN-WEEKLY-001), `TrainingLoadCore.sessionLoadSRPE/rollingLoadSum` (CALC-LOAD-003/005, venster 7 d/28 d caller-side), `ProgressionCore.trendBy` (CALC-LOAD-004) | **CALCULATED** (per contextopbouw, niet gepersisteerd; provenance = registry-ID + `confidence`/`n_performances` zoals de core ze levert; `insufficient` blijft expliciet insufficient) | **daily/session** | AI-promptcontext, per actieve sport gescheiden (running/cycling/swimming; triathlon alle drie; kracht → geen blok, nul queries); instructie "reeds berekend — niet herberekenen, geen ACWR-/blessure-interpretatie; ≠ profiel-drempels" | Geen (geen formule in Context; W′/D′/R² niet naar AI; ACWR niet geïnterpreteerd) |
