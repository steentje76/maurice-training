...
| DEC-RECADJ-001 | Herstelgebaseerde set/RPE-aanpassing | `progression_adjust.v1` | dagfactor (CALC-REC-002), spierherstel-%, subjectief gevoel, pijn | dagfactor, spierherstel-rijen, gevoel, pijn | factor<0.90 of "slecht"→ −1,5 RPE/−1 set; factor<0.97 of "matig"/laag herstel→ −0,5 RPE; anders geen aanpassing | `{rpeDelta, setsDelta, redenen[]}` (+ `inputs`/provenance via `recoveryAdjustmentForToday`) | **Product heuristic** — drempels zijn productkeuzes | JA — enige implementatie; sinds v4.69.87 (GAP-P3-032) één canonical toepassingspad voor Programma/Normal/Guided (`applySessionRecovery`/`applyRecoveryToGuidedPlan`); precedence: override OF basis → CALC-STR-002 → DEC-DETRAIN-001 → DEC-RECADJ-001 → prescription; expliciete gewichts-override is finaal (readiness past alleen sets/RPE aan) |
