...
**UPDATE (vervolg-sprint, nog NIET CLOSED):** Product Owner koos expliciet voor optie (b): een volledig gescheiden coach-authored/assignment/materialisatie-architectuur, met behoud van de `trg_set_user_id`-invariant. Geïmplementeerd: `coach_program_templates` (coach-owned), `coach_program_assignments` uitgebreid (template_id/materialized_program_id), en `materialize_coach_assignment()` (SECURITY DEFINER-RPC, uitsluitend aanroepbaar door de athlete zelf — de INSERT in `programs` gebeurt onder de sessie van de athlete, dus de trigger doet gewoon zijn normale, correcte werk, geen bypass). Vier kritieke scenario's live adversarial bewezen: volledige flow (`programs.user_id` correct op de athlete), coach kan niet zelf materialiseren, idempotentie, geen cross-user-toegang. **Nog NIET bewezen/gebouwd:** de daadwerkelijke content-materialisatie (template-`content`-JSONB naar `program_blocks`/geplande trainingsdagen) — een gematerialiseerd programma is vooralsnog leeg. Calendar/adherence hergebruiken de bestaande, canonieke engines (geen tweede implementatie), maar tonen pas zinvolle data zodra er daadwerkelijk geplande content is. **GAP-P2-023 blijft OPEN** totdat deze laatste stap end-to-end bewezen is (zie `docs/MS-F10-03_COACH_PROGRAMMING_ASSIGNMENT.md`).
