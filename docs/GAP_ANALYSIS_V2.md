...
**Closure-bewijs:** `core/fStructuredIntervalsB3Erg.test.js` 176/176 (3 sporten: Definition-round-trip, invariant/fail-closed, immutable snapshot, IntervalEngineCore-executie, incomplete finish, één sessierij met exercise_id/instance/intervals_detail, History planned-vs-actual, sport-eenheden, legacy ad-hoc). `migratie_v565.sql` additief/nullable.
**Priority:** P2 — gesloten. **Resterend (P4):** BikeErg-cadans/`stroke_rate`-semantiek en drag/weerstand als vrije tekst; erg verschijnt niet in `tkEnduranceCtxProject` (activities-only) — bewust, geen dual-write.
