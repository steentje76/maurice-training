...
### GAP-P2-031 — **CLOSED** (Structured Intervals B3, v4.69.92) — ERG STRUCTURED PERSISTENCE
...
**Besluit (PO):** Erg-actuals horen in het **sessions-domein** (`sessions.intervals_detail`, `erg_intervals_actual.v1`); activities/activity_laps blijft het model voor standalone endurance — geen universeel endurance-persistencemodel. Een structured Erg-Definition bevat exact één canonieke erg-oefening (`roeien`/`bikeerg`/`skierg`) + `interval_prescription.v1`.
**Closure-bewijs:** `core/fStructuredIntervalsB3Erg.test.js` **182/182** (3 sporten: Definition-round-trip, invariant/fail-closed, immutable snapshot, IntervalEngineCore-executie, incomplete finish, één sessierij met exercise_id/instance/intervals_detail, History planned-vs-actual, sport-eenheden, legacy ad-hoc, ownership). `migratie_v565.sql` additief/nullable.
**Remediatie (onafhankelijke audit, PR #354):** (1) reëel uitgevoerde sabotage — prescribed `target.pace` tijdelijk in actual `power_w` geschreven → bewezen FAIL (3 mislukt) → productiecode sha256-identiek hersteld → bewezen PASS (176/176); (2) ownership: geen RLS-policywijziging in de migratie, Erg-History-read op `training_instances` byte-identiek gescoped aan de reeds geaudite B1/B2-renderer, geen service-role-sleutel, sessierij-insert stuurt geen `user_id` mee. Restlimitatie expliciet vastgelegd: een live, twee-gebruikers Postgres-RLS-proef is nergens in deze repo uitvoerbaar (geen lokale Postgres/pgTAP-harness voor enige feature) — bovenstaande is het sterkste offline-bewijs binnen deze architectuur.
**Priority:** P2 — gesloten. **Resterend (P4):** BikeErg-cadans/`stroke_rate`-semantiek en drag/weerstand als vrije tekst; erg verschijnt niet in `tkEnduranceCtxProject` (activities-only) — bewust, geen dual-write; live cross-account RLS-proef blijft een repository-brede infrastructuurbeperking (niet B3-specifiek).
...
### GAP-P3-033 — **CLOSED** (Canonical Replacement/Add Prescription, 14 september 2026) — Swap/toegevoegde oefening omzeilde de canonical resolver
**Capability-ID:** F0.7L canonical weight resolver, CALC-STR-006, DEC-DETRAIN-001, DEC-RECADJ-001
**Current (vóór):** `confirmSwapExercise`/`execReplaceExercise`/`addExConfirm` zetten `suggestedWeight:null` → prefill = laatste gewicht van de (nieuwe) oefening zonder basis/detraining/readiness-gewichtsfactor; oude `_detrain`/`_weightOverride`-vlaggen liftten mee; Guided `replaceEx` zonder canonical prev.date (031a).
**Closure-bewijs:** `canonicalNewExerciseItem()` (intent van blok, load canonical voor nieuwe oefening), Guided `replaceExAsync` + `strengthBasisCache.prev`; `core/fReplacementPrescription.test.js` 47/47 (sabotage 1/1/3).
**Priority:** P3 — gesloten.
...
### GAP-P2-025 (F13 Post-Audit P1-10, GEIMPLEMENTEERD via B9-01) — Endurance datamodel (running/cycling/rowing/swimming)
**Current (bijgewerkt na B9-01):** het migratie-klare contract (oorspronkelijk uitgewerkt in `docs/F13_POST_AUDIT_P1_10_ENDURANCE_ARCHITECTURE_CONTRACT.md`) is daadwerkelijk geimplementeerd (`migratie_v533.sql`): `activities`/`activity_laps`/`athlete_endurance_profile` bestaan live, met SI-canonical units, provenance, RLS (adversarial bewezen), least-privilege-grants, en indexering vanaf dag 1. `CardioCore.criticalSpeed()`/`criticalPower()` (CALC-END-004/004B) blijven bewust NIET automatisch geintegreerd op de nieuwe activity-data -- de architecturele beperking (geen mechanisme om een genuine maximale-inspanning-tijdrit te herkennen) is niet opgelost, dat zou een aparte productbeslissing/UI-sprint vereisen. Running/Cycling-capability-scores blijven vooralsnog ongewijzigd (deze sprint bouwde de datalaag, geen UI/intelligentie) -- zie `docs/B9_01_ENDURANCE_DATA_FOUNDATION_REPORT.md` voor het volledige bewijs.
**Target (bereikt):** de canonieke datalaag bestaat nu live, klaar voor toekomstige Running/Cycling-intelligentie-sprints om op voort te bouwen.
**Priority:** P2 (architectuurwerk afgerond, geen actief security-/data-integriteitsrisico). **Complexity:** L. **Roadmap phase:** B9-01 (Benchmark 9.0 Floor Program), afgerond; vervolgsprints (B9-02 t/m B9-06) wachten op onafhankelijke review.
