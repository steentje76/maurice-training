...
`core/fDecisionRuleRegistry.test.js` (20/20): golden cases voor elke kernregel, determinisme-check, en een kritieke guardrail-test die bevestigt dat `readinessDay()` nooit een niet-gedefinieerde zone kan opleveren. Het sabotagebewijs (een verzonnen `'stop'`-zone geïnjecteerd) liet **fail-fast-gedrag** zien: de code crasht bij een poging tot een onbevoegde zone (geen tekst gedefinieerd voor `'stop'` in `READINESS_ZONE_TEKST`). **Correctie (MS-F3-09-reconciliatie):** deze crash zelf is géén ontworpen beveiligings- of veiligheidsmechanisme — het is de afwezigheid van een mapping. De daadwerkelijke garantie is dat de toegestane zones expliciet begrensd zijn (`READINESS_ZONES = ['ready','caution','reduce']`) en dat tests dit contract bewaken, niet dat een crash bescherming biedt. Volledig teruggedraaid en geverifieerd.
