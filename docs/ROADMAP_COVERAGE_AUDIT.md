...
**Canonical capability count: 38 (was 37 vóór MS-F3-11 — CALC-EVIDENCE-SPEC-001 toegevoegd).**

**Resultaat: 38/38 = 100%.** Alle 38 capabilities in `docs/CAPABILITY_REGISTRY.md` hebben een mastersprint-verwijzing in `docs/ROADMAP_INDEX.json` (zie `next_action`-veld per capability) én een eigen `capability`-type entry in `docs/ROADMAP_INDEX.json` (zie sectie "Capability-classificatie" hieronder voor de volledige lijst).
...
**Dekking: 38/38 capabilities geclassificeerd, 0 orphan** (Registry Coverage = 100%, zie hierboven).
