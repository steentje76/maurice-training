...
**Canonical capability count: 26 (was 25 vóór MS-F1-01 — SEC-USERROLE-001 toegevoegd).**

**Resultaat: 26/26 = 100%.** Alle 26 capabilities in `docs/CAPABILITY_REGISTRY.md` hebben een mastersprint-verwijzing in `docs/ROADMAP_INDEX.json` (zie `next_action`-veld per capability) én een eigen `capability`-type entry in `docs/ROADMAP_INDEX.json` (zie sectie "Capability-classificatie" hieronder voor de volledige lijst).
...
**Dekking: 26/26 capabilities geclassificeerd, 0 orphan** (Registry Coverage = 100%, zie hierboven).
