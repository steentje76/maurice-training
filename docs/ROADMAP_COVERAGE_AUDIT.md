...
**Canonical capability count: 34 (was 33 vóór MS-F3-07 — DEC-RULE-REGISTRY-001 toegevoegd).**

**Resultaat: 34/34 = 100%.** Alle 34 capabilities in `docs/CAPABILITY_REGISTRY.md` hebben een mastersprint-verwijzing in `docs/ROADMAP_INDEX.json` (zie `next_action`-veld per capability) én een eigen `capability`-type entry in `docs/ROADMAP_INDEX.json` (zie sectie "Capability-classificatie" hieronder voor de volledige lijst).
...
**Dekking: 34/34 capabilities geclassificeerd, 0 orphan** (Registry Coverage = 100%, zie hierboven).
