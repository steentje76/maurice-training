...
**Canonical capability count: 65 (was 64 vóór MS-F11-04 — GYM-DEVICE-FEASIBILITY-001 toegevoegd).**

**Resultaat: 65/65 = 100%.** Alle 65 capabilities in `docs/CAPABILITY_REGISTRY.md` hebben een mastersprint-verwijzing in `docs/ROADMAP_INDEX.json` (zie `next_action`-veld per capability) én een eigen `capability`-type entry in `docs/ROADMAP_INDEX.json` (zie sectie "Capability-classificatie" hieronder voor de volledige lijst).
...
**Dekking: 65/65 capabilities geclassificeerd, 0 orphan** (Registry Coverage = 100%, zie hierboven).
