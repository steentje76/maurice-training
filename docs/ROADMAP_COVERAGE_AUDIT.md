...
**Canonical capability count: 59 (was 58 vóór MS-F9-03 — SOCIAL-SHARING-NOTIFICATIONS-001 toegevoegd).**

**Resultaat: 59/59 = 100%.** Alle 59 capabilities in `docs/CAPABILITY_REGISTRY.md` hebben een mastersprint-verwijzing in `docs/ROADMAP_INDEX.json` (zie `next_action`-veld per capability) én een eigen `capability`-type entry in `docs/ROADMAP_INDEX.json` (zie sectie "Capability-classificatie" hieronder voor de volledige lijst).
...
**Dekking: 59/59 capabilities geclassificeerd, 0 orphan** (Registry Coverage = 100%, zie hierboven).
