...
**Canonical capability count: 30 (was 29 vóór MS-F3-03 — CALC-REC-REGISTRY-001 toegevoegd).**

**Resultaat: 30/30 = 100%.** Alle 30 capabilities in `docs/CAPABILITY_REGISTRY.md` hebben een mastersprint-verwijzing in `docs/ROADMAP_INDEX.json` (zie `next_action`-veld per capability) én een eigen `capability`-type entry in `docs/ROADMAP_INDEX.json` (zie sectie "Capability-classificatie" hieronder voor de volledige lijst).
...
**Dekking: 30/30 capabilities geclassificeerd, 0 orphan** (Registry Coverage = 100%, zie hierboven).
