...
**Canonical capability count: 37 (was 36 vóór MS-F3-10 — PROVENANCE-EXPLAINABILITY-001 toegevoegd).**

**Resultaat: 37/37 = 100%.** Alle 37 capabilities in `docs/CAPABILITY_REGISTRY.md` hebben een mastersprint-verwijzing in `docs/ROADMAP_INDEX.json` (zie `next_action`-veld per capability) én een eigen `capability`-type entry in `docs/ROADMAP_INDEX.json` (zie sectie "Capability-classificatie" hieronder voor de volledige lijst).
...
**Dekking: 37/37 capabilities geclassificeerd, 0 orphan** (Registry Coverage = 100%, zie hierboven).
