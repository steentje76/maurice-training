...
**Canonical capability count: 27 (was 26 vóór MS-F1-03 — SEC-CONFIG-001 toegevoegd).**

**Resultaat: 27/27 = 100%.** Alle 27 capabilities in `docs/CAPABILITY_REGISTRY.md` hebben een mastersprint-verwijzing in `docs/ROADMAP_INDEX.json` (zie `next_action`-veld per capability) én een eigen `capability`-type entry in `docs/ROADMAP_INDEX.json` (zie sectie "Capability-classificatie" hieronder voor de volledige lijst).
...
**Dekking: 27/27 capabilities geclassificeerd, 0 orphan** (Registry Coverage = 100%, zie hierboven).
