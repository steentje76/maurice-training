...
**Canonical capability count: 31 (was 30 vóór MS-F3-04 — CALC-END-REGISTRY-001 toegevoegd).**

**Resultaat: 31/31 = 100%.** Alle 31 capabilities in `docs/CAPABILITY_REGISTRY.md` hebben een mastersprint-verwijzing in `docs/ROADMAP_INDEX.json` (zie `next_action`-veld per capability) én een eigen `capability`-type entry in `docs/ROADMAP_INDEX.json` (zie sectie "Capability-classificatie" hieronder voor de volledige lijst).
...
**Dekking: 31/31 capabilities geclassificeerd, 0 orphan** (Registry Coverage = 100%, zie hierboven).
