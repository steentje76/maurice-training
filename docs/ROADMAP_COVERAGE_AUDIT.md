...
**Canonical capability count: 36 (was 35 vóór MS-F3-09 — EVIDENCE-CLAIM-AUDIT-001 toegevoegd).**

**Resultaat: 36/36 = 100%.** Alle 36 capabilities in `docs/CAPABILITY_REGISTRY.md` hebben een mastersprint-verwijzing in `docs/ROADMAP_INDEX.json` (zie `next_action`-veld per capability) én een eigen `capability`-type entry in `docs/ROADMAP_INDEX.json` (zie sectie "Capability-classificatie" hieronder voor de volledige lijst).
...
**Dekking: 36/36 capabilities geclassificeerd, 0 orphan** (Registry Coverage = 100%, zie hierboven).
