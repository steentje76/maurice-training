...
**Canonical capability count: 58 (was 57 vóór MS-F9-02 — SOCIAL-GROUPS-CHALLENGES-001 toegevoegd).**

**Resultaat: 58/58 = 100%.** Alle 58 capabilities in `docs/CAPABILITY_REGISTRY.md` hebben een mastersprint-verwijzing in `docs/ROADMAP_INDEX.json` (zie `next_action`-veld per capability) én een eigen `capability`-type entry in `docs/ROADMAP_INDEX.json` (zie sectie "Capability-classificatie" hieronder voor de volledige lijst).
...
**Dekking: 58/58 capabilities geclassificeerd, 0 orphan** (Registry Coverage = 100%, zie hierboven).
