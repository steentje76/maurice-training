...
**Canonical capability count: 51 (was 50 vóór MS-F7-01 — LONGITUDINAL-TREND-001 toegevoegd).**

**Resultaat: 51/51 = 100%.** Alle 51 capabilities in `docs/CAPABILITY_REGISTRY.md` hebben een mastersprint-verwijzing in `docs/ROADMAP_INDEX.json` (zie `next_action`-veld per capability) én een eigen `capability`-type entry in `docs/ROADMAP_INDEX.json` (zie sectie "Capability-classificatie" hieronder voor de volledige lijst).
...
**Dekking: 51/51 capabilities geclassificeerd, 0 orphan** (Registry Coverage = 100%, zie hierboven).
