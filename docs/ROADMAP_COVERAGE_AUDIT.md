...
**Canonical capability count: 53 (was 52 vóór MS-F7-03 — ADHERENCE-INTELLIGENCE-001 toegevoegd).**

**Resultaat: 53/53 = 100%.** Alle 53 capabilities in `docs/CAPABILITY_REGISTRY.md` hebben een mastersprint-verwijzing in `docs/ROADMAP_INDEX.json` (zie `next_action`-veld per capability) én een eigen `capability`-type entry in `docs/ROADMAP_INDEX.json` (zie sectie "Capability-classificatie" hieronder voor de volledige lijst).
...
**Dekking: 53/53 capabilities geclassificeerd, 0 orphan** (Registry Coverage = 100%, zie hierboven).
