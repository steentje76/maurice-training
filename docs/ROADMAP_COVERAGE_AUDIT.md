...
**Canonical capability count: 50 (was 49 vóór MS-F6-06 — SWIMMING-FEASIBILITY-001 toegevoegd).**

**Resultaat: 50/50 = 100%.** Alle 50 capabilities in `docs/CAPABILITY_REGISTRY.md` hebben een mastersprint-verwijzing in `docs/ROADMAP_INDEX.json` (zie `next_action`-veld per capability) én een eigen `capability`-type entry in `docs/ROADMAP_INDEX.json` (zie sectie "Capability-classificatie" hieronder voor de volledige lijst).
...
**Dekking: 50/50 capabilities geclassificeerd, 0 orphan** (Registry Coverage = 100%, zie hierboven).
