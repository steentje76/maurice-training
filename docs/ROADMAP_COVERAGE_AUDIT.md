...
**Canonical capability count: 54 (was 53 vóór MS-F7-04 — RELATIONSHIP-DISCOVERY-001 toegevoegd).**

**Resultaat: 54/54 = 100%.** Alle 54 capabilities in `docs/CAPABILITY_REGISTRY.md` hebben een mastersprint-verwijzing in `docs/ROADMAP_INDEX.json` (zie `next_action`-veld per capability) én een eigen `capability`-type entry in `docs/ROADMAP_INDEX.json` (zie sectie "Capability-classificatie" hieronder voor de volledige lijst).
...
**Dekking: 54/54 capabilities geclassificeerd, 0 orphan** (Registry Coverage = 100%, zie hierboven).
