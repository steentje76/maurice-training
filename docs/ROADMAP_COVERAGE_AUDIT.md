...
**Canonical capability count: 55 (was 54 vóór MS-F7-05 — ATHLETE-DASHBOARD-2-001 toegevoegd).**

**Resultaat: 55/55 = 100%.** Alle 55 capabilities in `docs/CAPABILITY_REGISTRY.md` hebben een mastersprint-verwijzing in `docs/ROADMAP_INDEX.json` (zie `next_action`-veld per capability) én een eigen `capability`-type entry in `docs/ROADMAP_INDEX.json` (zie sectie "Capability-classificatie" hieronder voor de volledige lijst).
...
**Dekking: 55/55 capabilities geclassificeerd, 0 orphan** (Registry Coverage = 100%, zie hierboven).
