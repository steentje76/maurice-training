...
**Canonical capability count: 44 (was 43 vóór MS-F5-01 — PROVIDER-INTEGRATION-CONTRACT-001 toegevoegd).**

**Resultaat: 44/44 = 100%.** Alle 44 capabilities in `docs/CAPABILITY_REGISTRY.md` hebben een mastersprint-verwijzing in `docs/ROADMAP_INDEX.json` (zie `next_action`-veld per capability) én een eigen `capability`-type entry in `docs/ROADMAP_INDEX.json` (zie sectie "Capability-classificatie" hieronder voor de volledige lijst).
...
**Dekking: 44/44 capabilities geclassificeerd, 0 orphan** (Registry Coverage = 100%, zie hierboven).
