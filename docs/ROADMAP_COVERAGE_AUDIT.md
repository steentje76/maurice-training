...
**Canonical capability count: 47 (was 46 vóór MS-F5-06 — WEATHER-CONTEXT-001 toegevoegd).**

**Resultaat: 47/47 = 100%.** Alle 47 capabilities in `docs/CAPABILITY_REGISTRY.md` hebben een mastersprint-verwijzing in `docs/ROADMAP_INDEX.json` (zie `next_action`-veld per capability) én een eigen `capability`-type entry in `docs/ROADMAP_INDEX.json` (zie sectie "Capability-classificatie" hieronder voor de volledige lijst).
...
**Dekking: 47/47 capabilities geclassificeerd, 0 orphan** (Registry Coverage = 100%, zie hierboven).
