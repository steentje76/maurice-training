...
**Canonical capability count: 49 (was 48 vóór MS-F6-02 — CYCLING-INTELLIGENCE-001 toegevoegd).**

**Resultaat: 49/49 = 100%.** Alle 49 capabilities in `docs/CAPABILITY_REGISTRY.md` hebben een mastersprint-verwijzing in `docs/ROADMAP_INDEX.json` (zie `next_action`-veld per capability) én een eigen `capability`-type entry in `docs/ROADMAP_INDEX.json` (zie sectie "Capability-classificatie" hieronder voor de volledige lijst).
...
**Dekking: 49/49 capabilities geclassificeerd, 0 orphan** (Registry Coverage = 100%, zie hierboven).
