...
**Canonical capability count: 41 (was 40 vóór MS-F4-03 — EXERCISE-PROGRESSION-COACH-001 toegevoegd).**

**Resultaat: 41/41 = 100%.** Alle 41 capabilities in `docs/CAPABILITY_REGISTRY.md` hebben een mastersprint-verwijzing in `docs/ROADMAP_INDEX.json` (zie `next_action`-veld per capability) én een eigen `capability`-type entry in `docs/ROADMAP_INDEX.json` (zie sectie "Capability-classificatie" hieronder voor de volledige lijst).
...
**Dekking: 41/41 capabilities geclassificeerd, 0 orphan** (Registry Coverage = 100%, zie hierboven).
