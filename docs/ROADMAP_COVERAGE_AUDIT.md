...
**Canonical capability count: 35 (was 34 vóór MS-F3-08 — DQ-CONFIDENCE-CONTRACT-001 toegevoegd).**

**Resultaat: 35/35 = 100%.** Alle 35 capabilities in `docs/CAPABILITY_REGISTRY.md` hebben een mastersprint-verwijzing in `docs/ROADMAP_INDEX.json` (zie `next_action`-veld per capability) én een eigen `capability`-type entry in `docs/ROADMAP_INDEX.json` (zie sectie "Capability-classificatie" hieronder voor de volledige lijst).
...
**Dekking: 35/35 capabilities geclassificeerd, 0 orphan** (Registry Coverage = 100%, zie hierboven).
