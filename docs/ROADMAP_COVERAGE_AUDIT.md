...
**Canonical capability count: 48 (was 47 vóór MS-F6-01 — RUNNING-INTELLIGENCE-001 toegevoegd).**

**Resultaat: 48/48 = 100%.** Alle 48 capabilities in `docs/CAPABILITY_REGISTRY.md` hebben een mastersprint-verwijzing in `docs/ROADMAP_INDEX.json` (zie `next_action`-veld per capability) én een eigen `capability`-type entry in `docs/ROADMAP_INDEX.json` (zie sectie "Capability-classificatie" hieronder voor de volledige lijst).
...
**Dekking: 48/48 capabilities geclassificeerd, 0 orphan** (Registry Coverage = 100%, zie hierboven).
