...
**I18n-supplement:** twee aanvullende geplande mastersprints zijn productmatig gereserveerd in `docs/I18N_ROADMAP_2_0.md`: MS-I18N-01 (Internationalization Foundation) en MS-I18N-02 (Dutch + English Complete Language Coverage). Zelfde regime als het Teamsport-supplement: nog niet in `docs/ROADMAP_INDEX.json`, niet IMPLEMENTED, integratie volgt bij dezelfde toekomstige canonicalisatiesprint.

**Telemetry/Beta-feedback-supplement:** drie aanvullende geplande mastersprints zijn productmatig gereserveerd in `docs/PRODUCT_TELEMETRY_FEEDBACK_ROADMAP_2_0.md`: MS-TELEMETRY-01 (Product Telemetry & Observability Architecture), MS-BETA-01 (In-App Beta Feedback & Triage Loop) en MS-BETA-02 (Release Intelligence & Closed-Beta Readiness). Zelfde regime: nog niet in `docs/ROADMAP_INDEX.json`, niet IMPLEMENTED, integratie volgt bij dezelfde toekomstige canonicalisatiesprint. Item-aantal van `docs/ROADMAP_INDEX.json` blijft dit sprint ongewijzigd op 119 (79 mastersprints + 40 capabilities) — zie `docs/DOCUMENTATION_GOVERNANCE.md` voor de precedentregel dat nieuwe productrichting eerst hier wordt vastgelegd vóórdat de machine-leesbare index verandert.
...
### 20B. Internationalization & Localization (i18n/l10n)
Trainingskompas becomes architecturally multilingual, with Dutch and English as the first complete locales. i18n/l10n is a cross-cutting capability (it spans Athlete UX, Exercise Library, Programs/Workouts, Decision explanations and AI Coach communication) and does not introduce a new product track — the 18-track model (§6) and its per-track mastersprint counts (`docs/ROADMAP_COVERAGE_AUDIT.md` §3) are unchanged. Canonical domain IDs (exercise/calculation/decision/evidence/context/program/workout/database/provider/analytics-event IDs, API contracts) remain language-neutral; only display-layer content is localized. AI language changes communication only — never calculation, threshold, decision outcome, evidence or confidence (direct extension of the AI Output Contract, §3/§16). Newly added, validated Exercise Library entries may become candidates for future AI program generation once sufficient validated metadata exists; the AI must not invent exercises outside the canonical library. Full scope, rules and proposed mastersprints (`MS-I18N-01`, `MS-I18N-02`, both PLANNED) are in `docs/I18N_ROADMAP_2_0.md`.

### 20C. Product Telemetry, Observability & Beta Feedback Loop
Product Telemetry is architecturally distinct from Athlete/Training Data and from explicit User Feedback (three separate data streams — see `docs/PRODUCT_TELEMETRY_FEEDBACK_ROADMAP_2_0.md` §3). This extends, and does not duplicate, the existing `MS-F1-02 — Observability Foundation` (capability `PLAT-OBSERVABILITY-001`, CLOSED/IMPLEMENTED, see `docs/OBSERVABILITY_CONTRACT.md`), which already covers structured crash/error events for AI Coach, wearable sync and platform flows. What is added here: product analytics as a governed, distinct stream; a canonical event registry; in-app beta feedback with a traceable triage lifecycle (SUBMITTED → TRIAGED → ACCEPTED/REJECTED/DUPLICATE → PRIORITIZED → PLANNED → FIXED → RELEASED → VERIFIED); and release-level go/no-go intelligence. Health/training data must never be forwarded as an analytics payload. No telemetry vendor is selected in this roadmap update (capability-based only, see `docs/PRODUCT_TELEMETRY_FEEDBACK_ROADMAP_2_0.md` §12). **Telemetry/Feedback must not be deferred until after Closed Beta** — that would forfeit the data the beta exists to collect; Closed Beta requires operational observability, minimal product analytics and feedback submission as a precondition (§13 of that document). Proposed mastersprints `MS-TELEMETRY-01`, `MS-BETA-01`, `MS-BETA-02` (all PLANNED, positioned in F13 Production & Scale) are detailed there.
...
**MASTER ROADMAP 2.0 v1.1 = CANONICAL SOURCE OF TRUTH.** Teamsport is als geplande productrichting geïntegreerd via T3/F6 (Teamsport Performance) en T15/F11 (Team Operations), met detailcontract in `docs/TEAMSPORT_ROADMAP_2_0.md`, inclusief een eigen primaire hoofdnavigatiebestemming (§20A/§1A aldaar). Internationalization/Localization (NL+EN eerste volledige talen, canonical IDs taalneutraal) is toegevoegd als cross-cutting capability, gedetailleerd in `docs/I18N_ROADMAP_2_0.md`. Product Telemetry, Observability-uitbreiding en de volledige Beta Feedback Loop (los van Athlete Data, met traceerbare triage-lifecycle, vóór Closed Beta) zijn toegevoegd, gedetailleerd in `docs/PRODUCT_TELEMETRY_FEEDBACK_ROADMAP_2_0.md`. Zie het finale canonicalisatierapport voor volledige validatiestatus.