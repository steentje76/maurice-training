...
async function openEngine(page){await page.setViewportSize({width:390,height:844});await page.goto(BASE);await page.locator('#projectHome .projectCard:not([disabled])').click();await expect(page.locator('#visualStudio')).toBeVisible();await expect.poll(()=>page.evaluate(()=>window.designRuntimeError?String(window.designRuntimeError):null)).toBe(null)}
