const assert=require('assert'),Core=require('./design-engine-core'),S=require('./screen-composer'),V=require('./composition-validation'),R=require('./design-repository'),P=require('./proposal-pipeline'),G=require('./generic-contract-exporter'),F=require('./freeze-readiness');
(async()=>{
 const files={
 ...
 const exporter=G.createGenericContractExporter();const pipeline=P.createProposalPipeline({composer,validator,repository,exporter,readinessValidator,productIdentity:active.pack.product_identity});
 ...
 assert.equal(bundle.freeze.status,'DESIGN_FROZEN');assert.equal(bundle.freeze.product_identity,'generic-product');assert.equal(bundle.contract.product_identity,'generic-product');assert(!bundle.freeze.freeze_id.startsWith('TK-'));assert.equal(bundle.contract.status,'BUILDABLE_FROM_FROZEN_DESIGN');assert.equal(bundle.contract.proposal.id,'E2E-1');assert(bundle.claude_prompt.includes('HARD GATE'));
 ...
 const badRepo=R.memoryRepository(),bad=P.createProposalPipeline({composer:S.createScreenComposer(badPack),validator:V.createCompositionValidator({rules:[]}),repository:badRepo,exporter});
