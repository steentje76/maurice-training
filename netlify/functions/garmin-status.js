// netlify/functions/garmin-status.js
// Devices/Wearables V1 MUST — Garmin. Geeft alleen onschadelijke metadata
// terug (wel/niet gekoppeld, laatste sync) — nooit access_token/
// refresh_token. Zelfde patroon als wearable-status.js (Google Health).
exports.handler = async function (event) {
  ...
  try {
    ...
    const r = await fetch(
      `${supabaseUrl}/rest/v1/wearable_connections?user_id=eq.${userId}&provider=eq.garmin&select=connected_at,last_sync_at,last_sync_status&limit=1`,
    ...
    return {
      ...
      body: JSON.stringify(row
        ? { connected: true, provider: 'garmin', connectedAt: row.connected_at, lastSyncAt: row.last_sync_at, lastSyncStatus: row.last_sync_status }
        : { connected: false, provider: 'garmin' }
      )
    };
  } catch (e) {
