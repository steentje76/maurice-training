...
exports.handler = async function (event) {
  ...
  try {
    ...
    const r = await fetch(
      `${supabaseUrl}/rest/v1/wearable_connections?user_id=eq.${userId}&provider=eq.oura&select=connected_at,last_sync_at,last_sync_status&limit=1`,
    ...
    return {
      ...
      body: JSON.stringify(row
        ? { connected: true, provider: 'oura', connectedAt: row.connected_at, lastSyncAt: row.last_sync_at, lastSyncStatus: row.last_sync_status }
        : { connected: false, provider: 'oura' }
      )
    };
  } catch (e) {
